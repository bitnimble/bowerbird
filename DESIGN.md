...
**Encode.** One `zscale` call applies the transfer for both media, then they diverge in chroma and container - but no longer in encoder. **Both are libaom in all-intra mode.** The still is 10-bit **4:4:4**, the video 10-bit **4:2:0**.

**`avifenc` is here for its container, not its codec**, and that is worth saying plainly because "avifenc for stills, ffmpeg for video" reads like an encoder choice. It is not: ffmpeg's avif muxer writes no `colr` box and AVIF has no equivalent of the bitstream filter that repairs the signalling on the video side, so avifenc is what puts an explicit nclx box on the file - which is what Chrome reads to decide a still is HDR. Underneath, both media are the same encoder.
...
**The y4m goes down a pipe, not through a file.** A still is converted by ffmpeg and encoded by avifenc, and the frame between them is uncompressed - 59MB at 3840, ~366MB at native resolution. Writing it out only to read it straight back is a round trip through the page cache, or through the disk on a machine short of it, for bytes neither process wants kept. `avifenc --stdin` takes it directly; the flag has to precede the output path and forbids an input one beside it.
