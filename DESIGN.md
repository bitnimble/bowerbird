...
Asked and answered rather than assumed, because the shape of the work invites it: the grade is a per-pixel gather and lookup, which is exactly what a GPU is for.

**AVIF encoding is the blocker, and it is a hard one.** AVIF is AV1 in a HEIF container, so it needs an AV1 encoder, and no hardware AV1 encoder does 4:4:4. NVIDIA's own support matrix gives AV1 as "YUV 420 8-bit and 10-bit" on Ada and Blackwell only - 4:4:4 exists for H.264 and HEVC but not for AV1 - and neither AMD's VCN 4.0 nor Intel's Arc QSV documents it either. This is the same wall §10.7 already hit from the other side: 4:4:4 is not AV1's Main profile, and no hardware *decoder* takes it. Since 4:4:4 is deliberate here and not a default (§10.1 - 4:2:0 smears the saturated edges a photograph is judged on), a GPU encoder would trade the thing the rendition exists to preserve for a stage that is 19% of the budget. libavif has no GPU backend upstream in any case.

**The decode splits into a GPU-friendly half and a GPU-hostile one, and the hostile half is bigger.** Demosaic is well served - NPP's `nppiCFAToRGB`, darktable's OpenCL kernels, Fastvideo's commercial CUDA implementations - but NPP's is bilinear with chroma correlation, which is *below* the PPG this uses, so the vendor-supported option is a quality regression. The unpack is the problem: ARW and CR2 carry Bayer data in lossless JPEG, whose Huffman decoding is bit-serial and a poor GPU fit. nvJPEG does not help, being a baseline DCT decoder rather than a lossless one. From the half-size measurements above, unpack is roughly half the decode, so the part that cannot move is the larger part.

**And the fit, the largest single consumer at 36%, is the wrong shape twice over.** There is no library for it, so it would be custom kernels; and it is dominated by a sequential refine whose every step depends on the last, which a GPU does not help. Only its candidate scan is parallel, and rayon shows that is worth ~3%.

So the reachable target is the grade (19%) plus a demosaic that is worse than the current one, against: CUDA as a hard dependency, NVIDIA-only, and a second implementation of the pixel maths kept in agreement with the CPU one - which has to stay, since the baseline exists for machines with no AVX at all. Meanwhile an import already saturates the CPU at ~2.4 img/s, so the honest ceiling is Amdahl's on the fraction that can move.

Cheaper levers first, in order: the fit's sequential refine (36% of the budget, no new dependency), and the AVIF encoder itself, where libheif can be pointed at SVT-AV1 instead of aom.
