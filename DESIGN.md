...
**In the component, not in a store.** A version for a photo nobody is looking at buys nothing, and a store of them grows with every photo an import touches - which then needs a cap and an eviction rule to stand in for what unmounting does for free. `EventsPresenter` keeps one subscription per photo currently on screen and drops it with the view; losing a version costs one revalidation and nothing else.
