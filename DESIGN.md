...
`raw_denoise_luma` and `raw_denoise_chroma` were library settings. A frame at 12800 and one at base ISO want different answers, so a single setting could only ever be right for one of them: `EditDoc` carries **`luminanceNoise` and `colourNoise`** instead, 0 to 100, both defaulting to **40** - which on the rendition path is exactly the denoise the reference ships, and what the settings they replaced were tuned to.

**A fixed slider position is already ISO-adaptive, and that is the thing to understand before touching either number.** The plane is normalised to its own fitted sigma before the shrinkage runs, so the luma value is a threshold *in units of this frame's noise* - "40" means the same relative shrinkage on a base-ISO frame as on a pushed one. What it is not is bounded in absolute terms: at ISO 25600 the fitted sigma is 3.5% of full scale against 0.3% at ISO 50, so the same relative threshold removes ten times the signal amplitude. That asymmetry is why the editor's top end is 0.4 rather than the reference's 1.5, and it is the argument any future "auto" has to answer - an auto that *raises* the amount with ISO would make the only bad case worse.
