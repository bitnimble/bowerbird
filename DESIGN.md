...
**The falloff stage (§10.8) has no counterpart here yet.** It would lift in principle - a linear-light gain is domain-independent in a way the curves are not - but this section fits its own colour from its own pairs, so the term has to be added to that fit rather than carried across, and it has not been. An HDR rendition of a frame whose body corrected illumination therefore keeps LibRaw's falloff where its SDR twin does not. Note also what the SDR `delta_e` now means on the route through here: §10.8 gates on a residual measured *with* the falloff, and `hdr.rs` takes only the geometry from that profile. The gate is therefore slightly more permissive for HDR than the correction it will actually apply - harmless, because the geometry it admits was chosen before any gain existed and this section scores its own colour independently, but it is no longer the same number for both.
