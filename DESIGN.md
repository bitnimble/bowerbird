...
**Encode.** Both media are **libaom in all-intra mode**, diverging in chroma and container: the still is 10-bit **4:4:4**, the video 10-bit **4:2:0**. The transfer is applied twice over, once each way - `tone::pq` for the still, in process, and a `zscale` call for the video, in ffmpeg - because only the video still leaves this process.

**PQ only, in process.** The still's linked path applies PQ and nothing else, which is right for a PQ rendition: the grade hands over Rec.2020 linear, and PQ's output gamut is Rec.2020, so all that is left is a transfer this crate owns and a matrix libavif owns. The **SDR reference is a different picture** and keeps ffmpeg: `zscale` converts its *primaries*, Rec.2020 to BT.709, and applies the sRGB transfer rather than PQ. Routing it through the linked path tagged a PQ-encoded, Rec.2020-primaried frame as sRGB/BT.709 and called it a control - white rendering at about half luminance, measured at 23.5dB against the binary where PQ scores 65.9dB. A gamut conversion is exactly the thing this was not supposed to reimplement, so the reference keeps the path that already owns one.
