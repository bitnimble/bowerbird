...
`↑`/`↓` walk the list, `→`/`←` open and close a folder (stepping out to the parent when it is already closed), `Home`/`End` reach the ends. Exactly one row is in the tab order at a time - the cursor - so tabbing past the scroller lands on it rather than at the top of the document, and focusing anything inside a row moves the cursor there so the ring and the focus cannot disagree. A menu or a rename field that has focus keeps the arrows, which is the widget doing its job.
