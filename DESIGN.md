...
The handle API still exists, used by nothing in `src/` - it is how the differential pins read samples back, and `rawshim_pixels.ts` and `jpeg_match_test_only.ts` are fenced out of `src/**` by lint. It goes when those tests are expressed as commands too.
...
The doors in the other direction live in **`rawshim_pixels.ts`** and **`jpeg_match_test_only.ts`**, and lint keeps them shut: `.oxlintrc.json` bans both from `src/**`, allowing them only under `test/` and `**/tests/**`. `pixels`, `imageFromRgb`, `hdrGradedSamples`, `decodeRaw` and the whole handle API are behind them, none with a production caller; they exist so an integration test can assert on what Rust produced, which is the one thing the rule cannot do without: that a written AVIF matches the decode it came from, that a JPEG-matched render differs from a plain one, that HDR grading is stable frame to frame.
