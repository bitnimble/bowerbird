...
The stamp is what makes each of those announceable rather than merely true: a client builds its URLs out of `date_reprocessed` (§13.5), so telling it about a file the row does not know about yet would have the next list read walk that URL back to the copy the browser already holds. A photo therefore announces twice during an import of a rendering library, and the second announcement costs a revalidation of a tile that did not change - a 304, against a grid that would otherwise sit on placeholders until every render finished.
