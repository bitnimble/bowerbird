...
**`requestedDetailId` is what the view is on; `detail` is what has arrived.** One store holds one detail, so everything derived from "the open photo" has to say which of the two it means, and the answer is almost always the former: the neighbours the arrow keys offer (`detailIndex`, or a step made before the first fetch lands goes to the photo already open and does nothing), which library's default applies, and whether a response that has just resolved is still wanted. A detail fetch is not ordered against the one before it, so every write after an `await` checks `isCurrent` first - without it, a straggler for a photo the reader has left replaces the one they are looking at, and the page then reports the photo in the URL as missing from the catalogue.
