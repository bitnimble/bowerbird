...
The HDR setting asks the reader to believe something they cannot check from the settings page, so `/hdr` makes the case in photographs: five raw files, each rendered twice, with a control that swaps the two **in place**. Side by side, the eye travels between the frames and the difference becomes a matter of opinion; in one place it is simply visible, which is why the page is a toggle rather than a pair.
...
**Both halves come out of one `runJob`** (`scripts/hdr-demo-assets.ts`), with an SDR target and an HDR one, at the settings the app ships with - `SettingsSchema.parse({})`, not a table of numbers copied into the script. The only value that is not a default is the size, 1200px rather than 3840. So the pair is one render, one colour fit and one grade, encoded twice, and the page is showing what an import would actually produce. A demonstration graded to win would be worth nothing here, and the way to not do that is to have no grading of its own to get wrong.

**The raw files are Play Raw submissions from discuss.pixls.us**, each licensed CC BY-SA by its photographer and credited under the picture it produced. They are cached outside the repository and only the renditions are committed - about 900kB for the ten - so a checkout needs neither 25MB per photograph nor a built `librawshim.so` to serve the page.

**The scenes were chosen by measurement rather than by subject.** What most raw files hold above diffuse white is a stop or two, because the photographer metered for the subject and the sensor saturated a little above it. Measured on the encoded renditions, the frames that actually use the headroom are the ones with a light source *in* them: the neon sign peaks near 4000 nits with 2.6% of the frame above white, and the LED-lit still life 6.7%, where the moon over the roofs has effectively none. That ordering is the page's argument as much as its illustration - the well-known losses are the smaller ones - so the copy says so rather than implying every photograph gains equally.
...
The Firefox rewrap is the photo view's, through `useHdrVideo`. Five of them mount at once here, so that engine holds five MP4s resident where the photo view holds one - a few hundred kilobytes, and the alternative is rewrapping on every press.

**The stops chart above the photographs is the one thing on the page with no measurement behind it**, and it says so in its own caption: the four ranges are approximations whose figures move with where you decide the useful range ends. It is drawn from one axis anchored at diffuse white rather than from four bars of independent length, because what matters is not that a JPEG holds fewer stops than a sensor but *which* stops it is missing - it simply stops at white, and everything to the right of that mark is a light source.

**It costs 882kB in every desktop and Android build**, since `public/` is copied into `web/dist` and that is `frontendDist`. Accepted rather than overlooked: the renditions are the page, halving them would show as encoder artefacts in exactly the highlights being argued about, and lazy-loading buys nothing for a reader who scrolls to the bottom.
