...
`defaultRendition` therefore reads `preview_source` off the **library**, found through the grid row's `library_id`, rather than the `default_rendition` the server puts on the detail. The server's answer is derived from exactly the same library setting (§13.2), so this is the same fact a round trip earlier - and it is per photo, which matters in an album spanning two libraries, where the detail on hand belongs to a photo from the other one.
...
Thumbnails are generated asynchronously, so a tile's first request can 404 while processing is still writing the file, and nothing in the page can know when that changes. **The server says so**: `ProcessingService` announces each photo whose renditions it has just written, and `GET /api/events` streams those announcements to every connected client as `event: thumbnail` with the photo's id (`EventsApi`). Views subscribe by photo id, so a tile hears about its own photo and is not woken by anyone else's.

**One version per photo, held by the view that displays it** (`useRenditionVersion`). The tile and the viewer append it to their URLs, which is the only thing that makes a rebuilt file visible to an `<img>` that has already decoded the old one (§13.5). Remounting the element is not an alternative: three fresh `<img>`s with the same `src` produce one network request between them, because the browser hands the later ones the copy already in its in-memory resource cache without revalidating. The URL itself has to differ.

Per photo rather than per rendition because the viewer warms both neighbours and holds no detail for them: one value per photo is the same whether a frame is being warmed or painted, so the warmed URL is the one that ends up on screen. A moment (`Date.now()`) rather than a server-minted stamp because all it has to do is differ from the last value this view put in a URL, and a page load starts over from the plain URL, where the ETag is authoritative again.

**On the presenter, not in a store and not in the view.** Not a store, because it is not catalogue data and nothing renders *from* it - it is a cache key, read only where a URL is built. Not the view either, though that was tried: the viewer warms both neighbours, and per-view state cannot hold the warm/paint invariant above, because the subscription that knew a neighbour's version is torn down in the same commit that starts the one that needs it. The reader then lands on the plain URL, which the browser answers out of the copy it already has - the file from before the rebuild, with nothing left to correct it.

So `EventsPresenter` holds both: a subscription per photo on screen, dropped with the view, and the versions themselves keyed by photo. Those are capped, evicting oldest first and never one being watched - dropping a version is right for a photo nobody is looking at, where the ETag settles it on the next request, and wrong for one on screen, whose URL would move back to the copy already in hand.

**The announcement is raised where a rendition is written, not where the queue keeps its books.** Two paths write one: the queue's result handler, and `runOneOff`, which serves both the viewer's on-demand build (`POST /photos/:id/renditions/:r`, including the force rebuild that deliberately rewrites a file behind an unchanged URL) and the grid tile repaired on a detail read (§13.2). Announcing from the queue alone left the second and third silent, and made the client that asked for a build stamp its own version. That is a version no *other* client browsing the same catalogue ever hears about, and none at all for the repair, which no client asks for.
...
The tile's retry and the announcement both feed the same suffix, and both are moments rather than counts, so the newer wins and neither can produce a URL the other has already used - which a URL that repeats itself would turn into a request the browser never makes.
