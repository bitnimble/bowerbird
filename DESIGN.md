...
**They are the one pair on the page not built from one set of samples**, and getting there took two wrong answers. Scaling all three channels by the step gives an 8-bit row that walks to white - correctly, since raising a channel already at its ceiling is the one move 8 bits does not have, so the other two rise instead - but it also gives an HDR row climbing towards white as well, only slower, and side by side a reader reads those as the same move at different speeds rather than as different moves. Holding the two minor channels fixes the HDR row and destroys the 8-bit one, which goes pale *because* those channels are rising; hold them and it is five identical patches.

The resolution is that these two do not have to agree. Everywhere else a pair is one render encoded twice, because the argument there is what a photograph loses in the smaller file. Here the argument is narrower - that one format can express something the other cannot at all - so each arm is authored for its own format: 8 bits pushes the whole triple towards white because that is its only direction, and HDR raises the dominant channel alone, putting the light into the colour. Measured, an HDR row ends on five times the light at a *higher* saturation than it started, 0.95 to 0.99. The note under the strips tells the reader they are not the same picture, since every other pair on the page is.

These are the one comparison on the page that is **side by side rather than swapped in place**: they are abstract, so there is nothing to lose track of while the eye travels, and the point is the two strips ending up different shapes rather than one frame changing.
