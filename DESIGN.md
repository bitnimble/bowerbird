...
**And the renditions themselves share a frame, not just the settings.** `hdr::Cut` is the photo carried to the point where only the display still differs: nits per channel, after the fit-to-size, the warp, the colour transform and the sharpen. Targets are ordered largest first; the first is cut off the base and every smaller one is a `downscale` of it, so it builds no warp table and runs no colour sweep of its own. What is left per rendition is the roll-off into its peak, the transfer and the encode.

The shared frame is stored **coded, as `u16` PQ**, and both halves of that were arrived at by measurement rather than taste.

*Coded* rather than linear, because the sharpen has to read it perceptually: stored as nits it cost a `pq` and a `pq_inv` per sample to get in and out of the filter's domain. Stored in PQ the sharpen's coding is the identity, and the roll-off reads a table indexed by the same signal - so **nothing on the shared path evaluates a transfer per pixel**. The write side is a lookup too, keyed on the `f16` the nits pass through, an `f16` having only 65536 values.

*`u16`* rather than `f16`, which is the opposite of what it looks like it should be. Half precision is right for **nits**, which span decades - a shadow at hundredths, a highlight at thousands - and 11 bits of significand is ~0.05% wherever the exponent sits. It is wrong for **PQ**, which already normalises to 0..1: the exponent then buys nothing and 11 mantissa bits leaves an ULP of ~0.001 near white, about 87 output levels. Measured, `f16` PQ cost the shared route mean 5.04 counts of 65535 where `f16` nits cost 0.71. A `u16` is the same width, uniform across exactly the range PQ occupies, and is what the HDR encode wants anyway.

`a_cut_rendition_is_the_picture_a_direct_grade_makes` holds the shared route against the direct one. Matched - what a library with the fit on renders - lands at mean **0.21** counts of 65535 into a 1000-nit peak and **1.02** into 203; the neutral fallback at 0.86 and 3.47. What is left is the one `f16` the nits pass through on their way to a code.
...
**The stages straddle the geometric warp, and which side each falls on is measured rather than chosen.** The denoise and the defringe run on the scene-linear frame *before* the warp and before the fit; the sharpen runs after the warp and the fit-to-size, on the frame at the size it will be encoded at.
...
All four run on perceptually-coded samples, never on linear ones: a difference taken in linear light is proportional to absolute luminance, so it treats a highlight and a shadow completely differently, and these all read differences. Which coding depends on where the stage sits, and neither costs a conversion pass (above). The first three run ahead of the warp on the shared base, in PQ against the frame's own diffuse white. The sharpen runs on the frame of nits every rendition is cut from, in absolute PQ - after the warp and the fit-to-size that apply the blur it deconvolves, and before any display's roll-off, which is what lets one sharpen serve every rendition of the photo.
