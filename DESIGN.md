...
The cost is that a photo can now have two geometry sources: HDR fits off the linear decode, SDR off the 8-bit one, since the SDR path needs the 8-bit render for its colour whatever happens. They agree to that 0.06%, and no job builds both at once - a renditions job carries a single `full` target, HDR or SDR - so the two only meet if the library's HDR setting is toggled and the stored copies compared.
