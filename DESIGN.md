...
**Both panel placements reserve their space, and the choice between them is made from the list rather than from the detail.** The strip under a landscape frame is a fixed `34vh` row, matching the fixed 320px column beside a portrait one; as an `auto` row it was zero high until the detail arrived and the photo jumped when the panels landed under it. Which edge they take needs the frame's shape, and waiting for the detail to say meant the same jump one level up - the loaded summary already carries width and height, so it answers immediately. Neither mattered until the stage started warming the next photo: a preloaded frame decodes the moment it is asked for, which is well before the detail request comes back.
