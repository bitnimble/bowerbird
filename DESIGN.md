...
**The stage prepares a frame while the detail is in flight but does not show it** (`hold`). Which edge the panels take comes from this photo's shape, and how tall the strip under a landscape one is comes from the panels' own content, so until the detail lands the stage is not the size it is about to be. That never showed until the stage started warming the next photo: a preloaded frame decodes the instant it is asked for, well before the detail request returns, so it painted and then jumped. Holding is preferable to reserving the space - a fixed strip is dead margin under every photo whose panels are shorter than it, which is most of them - and it costs nothing, since the frame is fetched and decoded meanwhile and goes up the moment the layout is settled.
