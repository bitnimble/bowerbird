...
**The swatch strips are the only synthetic thing on the page**, and they exist because every photograph below them assumes an answer to a question the reader has not been given: what "brighter" means once white is no longer the top. Five colours at a fixed hue and saturation, stepped up in light alone, encoded the same two ways as the photographs. In the 8-bit strip each row walks to white, not because anything desaturated it but because raising a channel that is already at its ceiling is the one move eight bits does not have, so the other two rise instead; the white row runs out entirely and its last three steps are the same white. They are 4:4:4 and lossless, since flat colour has no detail to trade and a quantiser on a hard edge between two saturated patches is visible where it is invisible on a photograph - which also means they skip the Firefox rewrap, which needs 4:2:0.
