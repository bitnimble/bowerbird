...
- **Rotate *history* away for a snapshot with no libraries that is smaller than it.** That is not a catalogue that shrank, it is a *replacement* for one: `createDatabase` creates on open, so anything leaving `DB_PATH` absent - a volume that failed to mount, a restore killed mid-rename - gets a fresh empty catalogue at the next start, whose snapshots would evict the real one's entire history within `keep` runs. At the defaults that is a week, with every run logging a successful backup. The test is emptiness rather than a size ratio because size cannot separate the two: an empty migrated catalogue is already 225KB of schema and a real but modest one only a few times that. Both conditions together, so a genuinely new install - empty, with no larger history to lose - still rotates normally.

  In that state rotation is restricted to the snapshots no bigger than the new one rather than stopped outright, which is what makes it safe to leave in place indefinitely. Removing the last library is something people legitimately do, and refusing to rotate at all would then accumulate a snapshot per run for ever, on the volume holding the catalogue, logging an error each time. The larger history is kept; the equally-empty ones rotate among themselves.
...
- Rotation keeps the newest N and reports how many it dropped; a retention below 1 deletes nothing; a snapshot with no libraries does not displace larger history, but a catalogue emptied on purpose still rotates among its own snapshots rather than accumulating them for ever
...
- A restore is refused while anything still holds the catalogue open, but a catalogue too corrupt for SQLite to open is still restorable over; a symlinked `DB_PATH` keeps the catalogue on the volume it was placed on, including when the link is dangling
- An unreadable backup directory does not stop a restore from an explicit path
