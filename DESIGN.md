...
- Restoring puts the catalogue back and the result is what a restart would find, with the displaced one complete and openable - the work its `-wal` held travels *inside* it, because the in-use probe checkpoints on its way past. The sidecar **move** is proved by the next case instead, where the probe returns early
...
- A bogus date is rotated out rather than made immortal; rotation never deletes the snapshot it just took; `latest` survives a backup directory copied without its timestamps; a chain of symlinks resolves to the catalogue at its end
