...
**They were side by side, and that turned out to be the reason a reader kept finding them wrong.** Three rounds of complaint about the HDR strip - lighter, then dark on the left, then dimmer in the first column than the 8-bit one - all traced to the same measurement, not to the samples. **A standard-range browser renders a PQ file by fixing its own white at about 406 nits.** Probed off the canvas and off a screenshot alike, a patch sitting exactly on diffuse white paints at 187 where an sRGB white paints 255, and nothing in the file moves it: `clli` makes no difference, and a flat 203-nit image with nothing above white at all still comes out at 187.

So any sRGB image shown *beside* a PQ one makes the PQ one look about a quarter dim, on the majority of screens, for a reason the page is not arguing. The strips are swapped in place now like everything else here, which removes the side-by-side entirely - there is nothing to hold the tone-mapped one against - and gets the strips the one idiom the rest of the page already uses. It also folds two copies of the same markup into one `Swap`.

The photographs never showed this because they were never adjacent. It is worth knowing anyway: it is what an SDR viewer sees of *every* HDR rendition this app writes, and it is why the notice at the top of the page is worded the way it is.
