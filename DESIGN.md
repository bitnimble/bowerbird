...
Together **1058 → 290ms against main's 119ms**, so the whole camera match costs about 170ms over an unmatched render rather than 940ms. Running the two arms under `rayon::join` was tried and measures 98ms *worse*: the batched probes already fill the cores and nesting only costs locality. Memory goes 627 → 673MB on a 24MP frame, all of it fixed-size buffers on the 640px fit grid rather than anything that scales with the picture - on a 61MP frame, where the decode sets the peak, it is unchanged.
