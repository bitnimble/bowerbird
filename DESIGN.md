...
There was a third cause, and it is the one that turned DSC05469's sky green. Gating each curve on its own channel fixed *which* pairs survive; it does not change that the fit domain ends where the **JPEG** clips that channel rather than where the camera's rendering does, and on a warm sky those are nowhere near each other - green's pairs ran out at render 0.18 and red's not until 0.66, with 116 of 87,000 samples above green's last filled bin. Three straight lines drawn from three different places diverge: by diffuse white the curves read 2.19 / 1.15 / 0.79, a 2.8× spread across channels on pixels well *inside* the trusted domain, and the held-out ΔE never saw it because that is measured only where all three channels have data. So a channel no longer extrapolates alone. **The channel whose pairs reached furthest carries the shape the others borrow above their own ends**, at the gain the two ran at where both had pairs - the channels agree on shape wherever they overlap, within about 5% across the domain on the fixture, so what a short channel lacks is reach, not a rendering of its own. Only above its own last bin: a channel keeps every pair it measured, and the borrowed part is the part that was a straight-line guess before.

That gain is read over the last 16 overlapping bins rather than at the join itself, which is the least trustworthy sample the channel has - the mask admits pairs up to sRGB 248, so a channel whose JPEG clipped early has the camera's 8-bit shoulder compressing its top bins, and a ratio taken there is carried over the whole tail. Replayed against a channel truncated at render 0.18, the window lands the extension within 1.9-2.1% of that channel's own curve where the join bin alone gives 3.2-4.0%, and self-extrapolation gives 33-38%.

**What still has no real-data coverage is the extension itself.** All three channels of the DSC02981 fixture fill every bin, so both extension paths run empty loops on it and the grade pin is byte-identical either side of this fix - which is also why `the_three_channels_leave_the_fit_domain_at_comparable_levels` never caught the green sky. The unit tests cover the algebra; a sky-heavy fixture is what would cover the case.
