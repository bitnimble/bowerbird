...
One number no longer means the same thing on both routes. §10.8 gates on a residual measured *with* the falloff, and `hdr.rs` takes only the geometry and the coefficients from that profile, never its ΔE. That is harmless - the geometry the gate admits was chosen before any gain existed, and this section scores its own colour independently - but the two ΔEs are not comparable and were never both about the same picture.
...
There was a third cause, and it is the one that turned DSC05469's sky green: **the binning threw away the evidence for the highlights, and each channel then extrapolated from wherever that left it.** That frame is bimodal, and measuring the mask's own decisions says why. 114K of its pairs sit below render 0.2. Its sky sits above 0.9 and is blown in the JPEG, so it is dropped - for all three channels alike, the earlier per-channel gating notwithstanding. The stretch between the two holds a few hundred pairs per tenth of the domain, most of them rejected for lying on a gradient, so whether a channel's last filled bin lands at 0.18 or at 0.66 comes down to which side of `MIN_BIN_SAMPLES` a hundred-odd survivors fall: red kept 113 pairs in the 0.6-0.7 band where green kept 13. Three straight lines drawn from three arbitrary places diverge - by diffuse white the curves read 2.19 / 1.15 / 0.79, a 2.8× spread on pixels well *inside* the trusted domain, and the held-out ΔE never saw it because that is measured only where all three channels have data.

**So the sparse tail is pooled rather than dropped.** Above the last bin dense enough to answer on its own, the remaining pairs are grouped in ascending order into however many points of `MIN_TAIL_SAMPLES` they can support, each group placed at its own mean. At a fixed bin width there is no floor that works for both ends of this frame - low enough to keep green's 13 and it keeps noise everywhere else - and a group that spans a wide stretch is the honest way to say what a scattering of pairs knows. On DSC05469 that takes green's own measured curve from render 0.18 to 0.75 and blue's to 0.74, and where the three read 0.79 / 1.23 / 0.58 at render 0.5 they now read 0.78 / 0.70 / 0.68. The sky stops being green because the frame is finally allowed to say what colour it is.

Above even that, a channel still does not extrapolate alone. **The channel whose pairs reached furthest carries the shape the others borrow above their own ends**, at the gain the two ran at where both had pairs - the channels agree on shape wherever they overlap, within about 5% across the domain on the fixture, so what a short channel lacks is reach, not a rendering of its own. Only above its own last bin: a channel keeps every pair it measured, and the borrowed part is the part that was a straight-line guess before.
...
**And the gain fades out across the tail, so by the top of the domain every channel *is* the reference curve.** A frame's brightest region is the one place the fit has direct evidence about, and on DSC05469 it is unambiguous: the sky is clipped in the *sensor* to exactly (1.000, 1.000, 1.000) and the camera renders it exactly (1.000, 1.000, 1.000). Neutral in, neutral out - which three curves holding three different gains up there cannot deliver, and did not: measured on the sky, U/V came out 506.0 / 514.0 against a neutral 512. Converged, a neutral render value maps to one number whatever channel it arrived in, and the same sky reads 511.9 / 512.0 with its mean saturation down from 5.96 to 0.13.

This is not desaturation, and the distinction is the whole point: what carries a highlight's colour is the pixel, not the curve. IMG_8584's twilight sky is genuinely blue (U 526.2, V 500.8) because its channels differ *in the render*, and it comes through this unchanged to four significant figures. What is given up is the fit's licence to invent a per-channel gain at a level where it measured none - and where a camera really does apply one, that is a real cost: on the synthetic chart, built with per-channel gains of 1.06 and 0.94, blue reads +4.4% at render 0.6 and +10.2% at 0.85.
