...
`raw_denoise_luma` and `raw_denoise_chroma` were library settings. A frame at 12800 and one at base ISO want different answers, so a single setting could only ever be right for one of them: `EditDoc` carries **`luminanceNoise` and `colourNoise`** instead, 0 to 100, both defaulting to **50**, which on the rendition path is exactly the denoise the reference ships.

**A position on the track is the fraction of the frame's own measured noise to remove**, and that is a statement about the scale rather than a figure of speech. The plane is normalised to its fitted sigma before the shrinkage runs, so 100 is the *calibrated* point where exactly the measured noise is treated as noise; past it the threshold is eating signal, so past it is not offered. A fixed position is therefore already ISO-adaptive - 50 is the same relative shrinkage at base ISO as on a pushed frame - with no auto needed to make it so.

**That only works because sigma is right, and getting it right was the harder half.** The estimator this shipped with took one median of the three-tap Laplacian over the whole frame, and on a detailed photograph the median pixel is not a quiet one: measured against a per-block envelope over a 32-frame library it reads **2.6 to 4.6 times high**. An over-estimate there is not a mild over-smoothing, because a block whose own deviation falls to the assumed noise has its whole AC zeroed rather than shrunk - so an inflated sigma flattens every block quieter than the texture that fooled it. The editor measures the envelope now, as Phase 0 does on the mosaic: each 8x8 block's own median Laplacian, then the quietest tenth of the blocks, since every block carries noise and only some carry a picture as well.

What is left for an **auto** is narrower than it looks, and it is not "more ISO, more denoise" - that rule picks 95 on an ISO 25600 frame, well past where its texture survives. It is a *gate*: `NoiseModel::suggested_amount` declines entirely below sigma 0.004, where a frame is clean enough that thirteen passes cost more than they remove, which is where this library's base-ISO frames sit.
