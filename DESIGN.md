...
Also swept here: a `<db>.restoring-<stamp>` left by a restore killed between its vacuum and its rename, which is catalogue-sized and which nothing else names - the same litter the backup side already sweeps for itself.
...
- A staging file left by a killed restore is swept by the next one; rotation does not delete the snapshot it just took even when the clock steps backwards mid-run
