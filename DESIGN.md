...
- A catalogue that has gone missing with backups beside it is refused at startup, naming the command that fixes it; a first run with no backups still creates one
...
- Restoring puts the catalogue back and the result is what a restart would find, with the displaced one complete and openable - the work its `-wal` held travels *inside* it, because taking the lock checkpoints on its way past. The sidecar **move** is proved by the next case instead, where the lock is skipped for want of a catalogue to take it on
