...
**Never its ΔE, but it used to inherit its verdict.** `MAX_ACCEPTABLE_DELTA_E` decides whether an *SDR render* should wear a colour transform; refused, §10.8 returns no profile at all. `fit_all` asked that same call for geometry and gave up when there was none - so on the one route an HDR rendition takes, a refused SDR colour refused the HDR match too, and the frame graded with no camera colour rather than with the fit that was never attempted. It is reachable rather than observed: the worst SDR fit across the 35-frame set is 4.1 against a limit of 6. It was reached by accident, feeding the geometry fit a cheaper preview, which took one frame's SDR fit to 10.4 and silently cost it its colour. `fit_ungated` is the same fit without that verdict, and geometry keeps its own test - a candidate still has to beat the undistorted baseline.

**One preview decode serves both halves, and which decode it is matters more than that it is shared.** Each used to decode the same 5-14MB preview to the same 640px grid - the colour fit through `thumbnail`, the geometry fit in full. Sharing the *cheap* one costs the 35-frame set **1.654 → 1.774** mean ΔE: `thumbnail` shrinks in the DCT before it builds a pixel, and the geometry fit is measurably worse off for it. Sharing the full decode leaves the set where it was and saves ~30ms of the fit's ~1.68s. Both directions were first measured one after the other rather than interleaved, which said the full-decode share was 34ms *slower*; the same build measured 1666ms and 1739ms in two sittings, so a difference that size was never there to read. Interleaved, nine runs each, it is a small win.
