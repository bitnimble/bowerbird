...
**The handle API is gone.** It outlived its last production caller by a while, because the pins had to assert on what the library produced and reading samples back was the only way to do it. It was not: everything those assertions actually check is a digest, a statistic or a comparison, and each is cheaper to compute where the pixels already are. `bb_debug` answers those questions in the same shape as `bb_run_job` - JSON in, JSON out - and `rawshim_pixels.ts` and `jpeg_match_test_only.ts` are deleted. Thirteen entry points remain, none of which returns a pointer, and `bb_free` and `bb_buffer_free` no longer exist because nothing is handed over to be freed. Marked unsafe operations went from 73 to 43 as a side effect.
...
The door in the other direction is **`rawshim_debug.ts`**, and lint keeps it shut: `.oxlintrc.json` bans it from `src/**`, allowing it only under `test/` and `**/tests/**`. It exists for the one thing the rule cannot do without - an integration test asserting on what Rust produced - and it does that without samples crossing. A written AVIF matching the decode it came from is a PSNR; a JPEG-matched render beating a plain one is two mean deltaE76s; a stable grade is a SHA-256. Each is computed natively and crosses as a number.

**One call does hand pixels over, and it goes through the filesystem.** `dumpDecode` writes a decode to a file for the single assertion that reads specific pixels rather than a statistic over them: `raw_header` checks four pixels, one per edge, for a black border. Opening a buffer path for it would reopen the thing this whole boundary exists to close, so it writes them somewhere and the test reads them back - and the native side logs a warning every time, so a production call shows up in a log rather than in a review that did not happen.
