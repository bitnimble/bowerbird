...
**Encoding.** SDR goes through libvips' AVIF encoder. The decode is deliberately 8-bit, because that encoder's output is 8-bit whatever goes in, so a 16-bit decode would be twice the memory for samples it discards. HDR goes through the path in §10.7 - libavif in process for the still, ffmpeg for the video - which is where 10-bit and the PQ transfer live.
...
**Both variants, but not by the same route.** `zscale` is zimg behind an ffmpeg filter: it converts transfer, primaries, matrix, range and depth in one pass, and each of those stages had an owner on this side already. PQ needs only the transfer, because the grade hands over Rec.2020 linear and PQ's output gamut *is* Rec.2020 - so it is `tone::pq`, the same curve the roll-off is computed in, and then libavif's matrix. The SDR reference needs its **primaries converted** as well, Rec.2020 to BT.709, and the sRGB curve instead: both of which `hdr_fit` has owned all along, because the fit measures its ΔE in sRGB. So nothing here is a reimplementation of zimg - the same stages are called from this process instead of another one.

That distinction is written down because getting it wrong is silent. Routing SDR through the PQ arm shipped a control that was PQ-encoded on unconverted primaries and tagged sRGB/BT.709: white rendered at about half luminance, measured at **23.5dB** against the binary. Done properly it scores **58.7dB**, against PQ's 65.9 - looser because a gamut conversion done by different arithmetic puts out-of-gamut colours wherever each side's clamp lands them, and an order of magnitude away from what a wrong curve scores.

**So the still never leaves this process**, either variant. `avifenc` and `ffmpeg` remain on that path only as the reference the pin runs against.
...
**It costs memory, and that is the trade rather than a footnote.** Three frames are live at once during a still encode: the graded buffer, the PQ-encoded copy, and the 10-bit planes libavif converts into. At 61MP that is ~1.1GB where the old path kept ~366MB on this side, because the other two lived in ffmpeg's and avifenc's address spaces and died with them. Multiply by `processing_concurrency`. Removing three transfers of a frame across a process boundary and keeping two allocations of it in the server is the shape of the deal; it is worth it at 24MP and worth measuring before raising concurrency on a machine building native-resolution HDR.
