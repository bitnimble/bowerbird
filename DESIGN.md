...
- **A Vulkan driver**, at runtime, because the grade is a shader and those shaders are its only implementation (§21.1) - `tone.rs`'s CPU grade was deleted rather than kept as a fallback. `apt install mesa-vulkan-drivers` covers AMD, Intel and **lavapipe**, the CPU rasteriser a machine with no GPU falls back to; NVIDIA wants its proprietary driver. Without any of them `job::run` refuses per job, naming what to install, and nothing renders.
  - In a container this is two more things, both in `docker-compose.yml`: `devices: /dev/dri:/dev/dri` and a `group_add` for the host's `render` group. Missing either is **not** an error - lavapipe picks it up and everything merely renders several times slower - so the entrypoint prints a `gpu:` line at startup naming the adapter it found, which is the only way to tell a misconfigured container from a machine that genuinely has no card.
...
- **A backup from a newer Bowerbird.** Restoring an *older* one is fine; the migrations run on the next start and bring it forward. The other direction is not: this build's migrations have no route to a schema they predate, and the failure would look like a corrupt catalogue rather than an error. `user_version` against `LATEST_USER_VERSION` is the check; and it is a **backstop, not a guarantee**. Only one migration stamps `user_version` at all; the rest recognise their own work from the schema, so a future build that adds no stamped migration produces backups this cannot tell from its own. Worth having because it costs nothing and catches the case where someone did bump it; not worth relying on.
...
**There is no second file for Firefox any more.** It used to be an opt-in second encode per photo - the same graded frame written again as a one-frame AV1 in an MP4, because Firefox applies a PQ transfer to nothing but video and renders an HDR still dark. Firefox is served the same AVIF as everything else now and rewraps it into that MP4 itself, in the page (§10.7). One setting, one file, one encode.
...
Read that as bounding *gross* change and nothing more. A mean ΔE76 is the one measure this design has repeatedly caught being blind to the errors that ruin a render - it discounts near-neutral error, cannot see a systematic cast at all, and hides its own tail (`fit.rs`, the `deltaE` section). What it says here is only that the new path did not move the colour by an amount that would show up even to a bad instrument. The reason to believe the rest is that both fixtures were rendered and looked at, before and against after, at full size and unmatched as well as matched. What does move is that an SDR rendition is a scene-referred render - diffuse white at the BT.2408 anchor, highlights rolled off by BT.2390 - where it used to be LibRaw's 8-bit output with its own auto-brightness. Held against a plain `decodeRaw` the two differ by a whole tone curve, which is why `lossless_render.integration.test.ts` asks the preview rather than the decode.
...
**The one decode takes it.** It used to pass 0 unconditionally and demosaic a 61MP frame in full, only for the grade to box-resize it to 3840 as its first act (§10.8.1) - fifteen sixteenths of the most expensive stage in the pipeline, thrown away. It asks for the largest edge any of the job's render targets wants, and a `max` target still reports 0 and is never halved. Two *jobs* over one photo therefore anchor their grades on decodes of different resolutions, and since the grade's anchor comes from the frame, a `full` and a `max` built separately can land on slightly different tone curves. Measured by grading both to the same output size: a **0.32% difference in mean brightness**, 0.2% RMS. Within one job it cannot happen at all, the levels being read once off the shared decode (§10.3).
...
**One decoder, for everybody.** There were two for a while - this one behind the server's fit and the browser's own behind the editor's - which is the divergence `image::resize` was introduced to remove. The editor's went with the wasm build (§21): the page fetches the RAW and prepares it through `edit::fit`, which reads the embedded preview through this module, so nothing on a client decodes a JPEG in JavaScript any more - the decoder a tab runs is this one, compiled to wasm.
...
**An encoded file has nothing to carry.** That route needs no sink - encode, mux, blob, `<video src>` - and `VideoEncoder` does exist, contrary to what this section used to say. But `VideoFrame` takes only `I420`, `NV12`, `RGBA` and `RGBX`: every 10-bit format is rejected (`I420P10 is unsupported`), 4:4:4 with them, and `P010` is not in the enum at all. So the encoder can only ever be fed 8 bits, which is the same ladder `pack.rs` dithers for, and it would cost an AV1 encode per slider tick to find out whether Gecko routes the result to the compositor. Canvas is no way round either, since `display-p3` silently resolves back to `srgb` and `rec2100-pq` is not in `PredefinedColorSpace`.
...
So an HDR rendition is **one** file, a PQ AVIF, and Firefox is handed that same file and rewraps it as a video in the page (§10.7.2). It used to be two, the second a one-frame AV1 in an MP4 that the import encoded and stored per photo.
...
**Encode.** **libaom in all-intra mode**, 10-bit, 4:2:0 by default with a setting to raise it. There were two media here and they diverged in container and in whether their chroma was negotiable; the video's was not, because nothing decodes 4:4:4 AV1 video, and that constraint left with it.

**The PQ transfer is applied once, on this side** (`tone::encode_pq`). It used to be applied wherever the encode happened - by libavif's caller for the still, by a `zscale` call in ffmpeg for the video - which is the same curve computed twice and, worse, left the two encoders holding frames in *different domains* between the grade and the encode. Nothing that belongs in between could then be written once, which is what the output sharpen (§10.9) needs. It stays here with the video gone: the reference path's `zscale` is told `tin=smpte2084` and does no transfer work at all, so what remains there is the YCbCr matrix, the range and the depth, which is the whole point of keeping that arm as a check on this one.

It cost nothing to move and settled a question the differential had been living with. `avif_still.integration.test.ts` finds the two still routes **byte-identical at 4:4:4** - both handed the same PQ samples, both applying the same matrix - where they used to sit ~60dB apart on the intermediate quantisation. 4:2:0 still parts company, zscale and libavif subsampling chroma their own ways. That took the differential's own guard with it: "not infinite" was how it proved its two arms had really taken different routes, so the route is now reported out of the library and asserted directly.
...
- **The transfer** allocated another. Same argument, and there is no longer a caveat: it runs once in the graded buffer before the encoder is handed anything, where it used to copy rather than transfer the samples out from under the video twin sharing them. `encode_still` keeps its `Cow` for the other half of the argument - an owned frame goes into libavif and is dropped as soon as the YUV conversion has read it.
...
This app does not use `install()`: React owns those nodes and the stage already knows how to show a video, so `web/src/features/photos/hdr_video.ts` takes only the URL from `hdrVideoUrl()` and hands it to the element the stage was going to mount anyway. The object URL is revoked the moment that element points elsewhere - the whole MP4 is resident until it is.
...
A job has one geometry source now: `fit_all` off the linear decode, whatever ranges its targets want (§10.3). The second route - fitting off a full-resolution 8-bit render - survives only where something already holds a `fit::Profile`, which in the product is nothing: it is `fit_profile_for`, reached by the debug commands and the pins. **An HDR rendition and its SDR counterpart are the same picture through the same pipeline as far as the transfer**, which is a change: they used to be different pictures built by different code, and agreement was explicitly not a goal.
...
**One preview decode serves both halves, and how far it was DCT-shrunk matters more than that it is shared.** Each used to decode the same 5-14MB preview - the colour fit bounded, the geometry fit in full, both landing at 640. Sharing was tried at 640 and rejected: a bounded decode shrinks in the DCT before it builds a pixel, so at that size it arrives barely filtered and the geometry fit is measurably worse off, costing the 35-frame set **1.654 → 1.774** mean ΔE.
...
**There used to be a second encode to overlap with**, the Firefox video twin, and the shape of that optimisation is worth keeping even though the twin is gone (§10.7.2): one graded frame served both media and the two encoders ran concurrently under a `thread::scope`, both reading that frame and neither writing it, so the pair cost about what the slower of them cost alone. Encoding them sequentially had regraded the frame for the second *and* serialised the encoders - measured on a 24MP frame at 3840, in-process CPU 649ms against 331ms while wall time barely moved. The saving was in CPU rather than wall clock, which is exactly what an import queues for at `processing_concurrency` workers, and the same reason a stopwatch on one photo could never show it.
...
**Verified against the reference rather than by eye.** `examples/galosh_parity.rs` runs the same synthetic frame through this port and through the reference's C build at two strength pairs: **69.3 dB and 69.0 dB**. The reference reports 69.7-70.6 dB between its *own* CPU and Vulkan builds, so this port is **0.4 to 1.6 dB short of that** - the same order of agreement rather than a match, which is what a second independent GPU implementation should look like and is stated that way rather than rounded into "within the band".

Fifty dispatches of compensated summation are not bit-exact across two compilers and asking them to be would be asking the wrong question. What the figure is *for* is the failure a transcription actually has - a transposed index, a sign, a phase skipped - and those land tens of dB below, so it separates the two cases with room to spare. It does not certify the last dB, and nothing here should be read as claiming it does.
...
`raw_denoise_luma` and `raw_denoise_chroma` were library settings. A frame at 12800 and one at base ISO want different answers, so a single setting could only ever be right for one of them: `EditDoc` carries **`luminanceNoise` and `colourNoise`** instead, 0 to 100, both defaulting to **40**.

**The scale's landmark is its middle.** The plane is normalised to its fitted sigma before the shrinkage runs, so a position on the track is the fraction of the frame's own measured noise to remove, and **50** is the calibrated point where exactly that much is treated as noise - which on the rendition path is also exactly the denoise the reference ships. A fixed position is therefore already ISO-adaptive, the same relative shrinkage at base ISO as on a pushed frame, with no auto needed to make it so.
...
The defaults sit well short of calibrated - **20 for luminance and 30 for colour** - because the failures are not symmetric in either direction. Short of the mark a frame keeps grain and past it the frame smears, and only the first still reads as a photograph; and between the two sliders, grain in luma has that defence where colour mottle has none, so the colour half can start further along than the one whose over-reach costs texture.
...
What is left for an **auto** is narrower than it looks, and it is not "more ISO, more denoise" - that rule picks 95 on an ISO 25600 frame, well past where its texture survives. It is a *gate*: `NoiseModel::suggested_amount` declines entirely below sigma 0.004, where a frame is clean enough that a full denoise costs more than it removes, which is where this library's base-ISO frames sit.

Separate, because the two answer to different complaints. Grain in luma reads as a photograph and is worth keeping some of; colour mottle has no such defence and wants all the smoothing it can be given. In the kernels they are genuinely different knobs rather than one scaled twice: the luma number is a shrinkage threshold in units of the frame's own fitted noise, and the colour number is a walk along three anchors - noisy, a half-resolution regression and a quarter-resolution level - so its unit is *how far colour may be smoothed, in scales*.
...
The mosaic never crosses to the client. So a Detail slider cannot re-run *this*, and the editor runs the reference's **sRGB front-end** instead, on the prepared frame, per tick - five small kernels plus the three heavy ones this path already has.

**The two are not expected to agree.** The editor trades quality for latency: one sigma split by a fixed ratio where this fits shot and read noise separately, two chroma channels where this has three transform terms, no per-slot dark reference because a demosaiced frame has no slots. The preview is an honest approximation of the export, and where the difference matters the loupe is the place to show the real thing.
...
What fixes it is the frame's fit, and the only host that has one is the client: `edit::prepare` measures it (`galosh::Fit::Only`, the same chain stopped after Phase 2, so it is a prefix of the denoise rather than a second estimator that agrees today), sends it on the prepared header as seven numbers, and the editor hands it straight back on each tile request. Nothing between the two ends reads it. The alternative - storing it against the photo - was measured and rejected: it is 28 bytes, but a fit belongs to a decode rather than to a file, and a row that has to be invalidated when the decoder changes is a cache. The tile route is a pure function of its query and stays one.
...
|---|---|---|
| `tone::levels` - the white it is coded against, the peak it rolls into | **9333** | the editor measures them at the open and sends them per tile |
| `peak.wgsl`'s scene peak, which the roll-off compresses into the display | **1343** | the tick measures it every draw; the loupe reads it back and sends it |
...
- Restoring an *older* backup is allowed; one with a `user_version` ahead of this build is refused, as is one that is not a readable database and one whose path is the catalogue itself; all before anything on disk moves
...
**And they accumulate over the photo, not over the rendition.** Which neighbour URLs those are moves with the rendition on screen, so holding only the current set meant every swap unmounted one pair and mounted the other - warming a file and then dropping the element holding it, which is warming nothing. It cost most in the case the warming exists for: comparing two renditions and then stepping on, where the frame arrived at had been fetched already and was fetched again. The set is cleared on the photo, where these are different photographs and what is held is next to nothing.
...
**Settings cross as one JSON value, `wasm::EditorSpec`.** The editor used to inline the shipping defaults for the peak, the anchor and the denoise strengths, so a library whose settings had been moved showed one picture in the viewer and wrote a different one to disk. The web side builds it in `editor_spec.ts` from the same settings the server reads.
...
That leaves a landmine: turning `+simd128` back on re-breaks the fit, invisibly and only in a browser. The warning sits on `fit::blur`, at the call, rather than in the build script. The guard is the e2e assertion that the browser reports `camera match` rather than `neutral`, since nothing native exercises the wasm kernel.
...
- **libblur is flag-gated** (`cfg(all(target_arch = "wasm32", target_feature = "simd128"))`) but runs once per open, inside the fit, so it cannot show up in a per-tick figure either way - and its flag-gated path is the broken one above.
- **LibRaw, libaom and libavif are not vectorised at all**, and deliberately - see `build_wasm_libs.sh`. Enabling `-msimd128` across the three raises the count to 465k and makes everything 30-130% slower, the still route's pure-Rust drag included, because the module grows 2MB and the engine's cost for that exceeds what the lanes save.
...
### 21.2 Three engines, three routes (`raw_edit_route.ts`)
...
- **Firefox: a 10-bit AV1 per tick, rewrapped as an MP4.** Gecko composites HDR through video and only video, and every in-page route to a video frame is capped at 8 bits - which it then will not composite either. So the frame is encoded properly and handed over in the container Gecko will take. See below.
...
The rewrap that serves Firefox its *renditions* (§10.7.2) works because the 10-bit AV1 already exists inside the AVIF and only the container changes. **A live edit has nothing to rewrap**, so it has to make one.
...
`RawEditStore` holds what the view reads; `RawEditPresenter` owns the worker, the track and every object URL, and is the only thing that writes. The pair is constructed when edit mode is entered on the photo detail page, and torn down when it is left or the photo changes - never for an ordinary viewer visit - rather than in the app's provider (§18.2), because it owns a worker and a few hundred MB of wasm heap. A new route rebuilds both, since the route is fixed at decode.
