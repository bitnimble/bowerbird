...
That machine reports `(dynamic-range: high)` **false** and `(video-dynamic-range: high)` **true**, which is the whole situation in two media queries: Gecko composites HDR for video and only video (bug 1889288), so the sole thing that reaches the panel is a video file its own pipeline decoded - the AV1 twin of §10.7, which a server encodes. A PQ-tagged AV1 shows the same flat-grey value as a BT.709-tagged one, so the tag buys nothing beyond that.
