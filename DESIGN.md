...
**Fitting the model twice and searching two parameters made the measurement itself the cost.** The fit now measures its own error upwards of a hundred times per photo - every ridge candidate at every round, every step of the saturation search, both arms of the shape question - where it used to do it once. Two things in that loop were being recomputed for nothing: the camera's sRGB target, which is three `powf` per pixel and never changes, and `rec2020_to_srgb`, a pair of 3×3 multiplies that was being rebuilt *per pixel per pass*. Hoisting both and running the measurement across cores took a 61MP HDR rendition from 8.3s back to 3.7s against an unmatched 3.3s. The blocks are summed in a fixed order rather than reduced: floating point addition is not associative, so a reduction whose tree depends on how `rayon` scheduled the work gives a different fit run to run, and the graded output is pinned by hash.
...
At the rendition size it no longer costs anything measurable, and at native resolution it costs 33MB rather than a gigabyte. At `raw_denoise` 3, the schema's maximum, native resolution measures **563MB** - the same, where before the budget was solved properly it gained another ~205MB. The duplicated work at the seams - each strip also computes its halo and throws it away - is **~3%** of wall clock.
...
| `raw_denoise` | `0.5` | Strength of the guided-filter denoise on a rendered RAW, regularised by the frame's own measured noise; 0 is off (§10.9) |
