...
**Both decodes take it, the scene-linear one included.** The HDR decode used to pass 0 unconditionally and demosaic a 61MP frame in full, only for the grade to box-resize it to 3840 as its first act (§10.8.1) - fifteen sixteenths of the most expensive stage in the pipeline, thrown away. It asks for the largest HDR edge the job wants, exactly as the SDR decode asks for the largest SDR one, and a `max` target still reports 0 and is never halved. The two renditions therefore anchor their grades on decodes of different resolutions, which is the same thing the SDR path has always done and is why the levels are read per decode rather than shared across jobs.

A third caller wants neither: on an all-HDR job nothing renders SDR, which leaves the camera-match fit as the 8-bit decode's only reader. It resizes to its own 640px grid before it looks at anything and the transform it returns is resolution-independent, so that decode asks for 640 rather than the whole frame it was previously demosaicing to feed a search that would immediately throw it away. This was not a decision so much as an accident: `largestSdrSize` over an empty list is `Math.max()`, which is `-Infinity`, which the decoder clamped to "the whole frame".
...
**Encode.** One `zscale` call applies the transfer for both media, then they diverge. The still is 10-bit **4:4:4**, encoded and muxed by **avifenc** (libaom) rather than by ffmpeg, because ffmpeg's avif muxer writes no `colr` box and AVIF has no equivalent of the bitstream filter to repair one; `--jobs all` matters more than any other flag, since avifenc is single-threaded by default and that alone is 9.6s against 0.5s on a 24MP frame. The video is 10-bit **4:2:0** via **SVT-AV1**, which is 2.4x faster than libaom and the only one of the two that carries the mastering-display and content-light metadata through ffmpeg.

**A correction worth keeping.** This section previously said both media were libaom at 4:4:4 and that SVT-AV1 "cannot be used here at all". Neither is what ships. The still is 4:4:4 because it is a photograph and 4:2:0 smears exactly the saturated edges one is judged on; the video is not, because 4:4:4 video was tried and reverted - it is AV1 Profile 1, Chromium refuses it outright, Safari cannot hardware-decode it, and on Firefox/Windows it played but rendered washed out, which is what a decode that never reaches the HDR compositor looks like. Profile 0 is all the video ever needed, and Profile 0 is all SVT-AV1 does. The paragraphs below about its height ceiling and its `-svtav1-params` metadata describe the encoder actually in use, not one that was dropped.

**4:4:4 is not a setting, it is an AV1 profile**, and that decides the encoder. Profile 0 is 4:2:0, Profile 1 is 4:4:4, Profile 2 is 4:2:2. SVT-AV1 implements Profile 0 only and *converts silently* - asking it for 4:4:4 or 4:2:2 yields 4:2:0 with no error - so it can only ever serve the video. libaom and rav1e implement all three, which is why the still goes through avifenc. Measured decoder support:
...
The **mastering-display and content-light metadata** reaches the video through `-svtav1-params` and has no libaom equivalent, so the still carries none. Those are tone-mapping hints, and Firefox 153 does no tone mapping, so nothing that consumes either file reads them. The load-bearing signalling is the CICP, which both carry.
...
**A browser will not scroll as far as a collection can reach**, and it truncates silently: Chromium clamps at 33,554,428px and Firefox at roughly half that, with everything past the clamp simply unreachable. That is not a millionth-photo problem - the grid at its highest zoom is one column of thousand-pixel rows, which runs out at **thirty thousand photos**, and list mode at half a million. So past `MAX_SCROLL` the scroller is compressed and positions are scaled into it: the layout stays in content pixels, `scrollScale` converts to scroll pixels at the one boundary where the element is read or written, and `domTop` places the rendered window where the browser thinks the reader is rather than where the row truly sits. Below the threshold the scale is exactly 1 and nothing is converted at all. The cost is that a wheel notch covers more of the collection once compression is on, which is what compressing a scroll means; the alternative was losing 69% of a hundred-thousand-photo library at maximum zoom.

**What a screen reader is told does not depend on what is mounted.** The scroller is a labelled `list` and a tab stop of its own - it holds content no other tab stop reaches, so Page Up/Down, Home and End would otherwise have nothing to act on - and every tile carries `aria-setsize` and `aria-posinset` against the *collection*, not against the few dozen tiles in the DOM: a reader is told "photo 40,051 of 100,000" rather than "photo 4 of 30". The spacer and the window between them are `presentation`, so the items stay the list's own children, and a position still waiting on its block is `aria-busy` rather than absent.
