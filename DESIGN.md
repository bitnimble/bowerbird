...
**The stops chart above them had no measurement behind it, and now two of its four bars do.** The file figures were folklore - "about 8 stops for a JPEG, about 15 for 10-bit" - until a reader asked where they came from, which was nowhere. They are computed now, on one criterion applied to both: how far down the range you get before the gap between adjacent code values exceeds 5%, which is roughly where a step starts to show as a band in a smooth gradient. Decoding sRGB that way gives **6.0 stops**, code 255 down to 33; decoding 10-bit PQ from 1000 nits gives **14.6**, down to 0.04 nits.

So the JPEG bar was a stop or two generous and is now 6, and the 10-bit bar was right. Both are sensitive to the criterion, which is why the caption states it: at 2% they are 2.8 and 10.3, and ignoring banding entirely the formats *encode* 11.7 and 24.6. What survives every criterion is the gap between them, which is the only thing the chart is claiming.

The other two bars cannot be computed and are labelled as approximations: a full-frame sensor's engineering dynamic range at base ISO, and the eye across one scene with the gaze moving. The chart is drawn from one axis anchored at diffuse white rather than from four bars of independent length, because what matters is not that a JPEG holds fewer stops than a sensor but *which* stops it is missing - it simply stops at white, and everything to the right of that mark is a light source.
