...
The map is capped at a couple of thousand photos and drops the oldest first, because an import announces every photo it touches and the tab may be open for all of it. Dropping one is safe rather than merely tolerable: losing a version returns that photo to its plain URL, where the ETag decides (§13.5).
