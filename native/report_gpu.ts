...
//
// **Because the failure this catches is silent.** The image carries lavapipe, so a
  ...
  if (length < 0) {
    console.log(
      'gpu: no Vulkan adapter answered, so no rendition will build. ' +
        'Install a driver - mesa-vulkan-drivers covers AMD, Intel and the CPU fallback',
    );
    ...
    // Named rather than inferred: `Cpu` is what wgpu calls lavapipe, and on a host with
