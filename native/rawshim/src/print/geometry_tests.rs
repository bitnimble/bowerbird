...
fn print_bends_along_its_long_edge_and_keeps_stock_thickness() {
    ...
    for shape in [[300.0, 200.0], [200.0, 300.0]] {
        ...
        let samples = probe(&scene, &[
            sample(project([0.0, 0.0, 0.001], &scene), shape),
            sample(project(edge, &scene), shape),
        ]);
    ...
    let sample = probe(&back, &[sample(project([0.0, 0.0, -0.001], &back), [300.0, 200.0])])[0];
...
fn print_edges_cover_fractional_pixels_at_front_and_grazing_angles() {
    for yaw in [0.0, 75.0, 89.8, 90.0, 180.0] {
        ...
        let edge = project([1.0, 0.0, 0.011], &scene);
    ...
    let center = project([0.0, 0.0, 0.0], &scene);
    ...
    let thin = project([0.0, 0.5, 0.0025], &scene);
...
fn print_texture_derivatives_follow_the_curved_tangent() {
    ...
    let center = project([0.7, 0.1, 0.0059], &scene);
    let samples = probe(&scene, &[
...
fn project(point: [f64; 3], scene: &Scene) -> [f32; 2] {
