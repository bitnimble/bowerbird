...
///
/// Hand-rolled for the same reason as the SHA-1 beside it. `hdr_grade.pin.txt` recorded
/// one per case when this was written and does not any more - it holds sizes, per-channel
/// statistics and a sampled row, which say *where* two grades part company where a digest
/// only says that they do. What still wants a digest is
/// `the_neutral_grade_is_reproducible_and_differs_from_the_matched_one`, which asks whether
/// two whole frames are identical and never wants to know more than yes or no.
#[cfg(test)]
