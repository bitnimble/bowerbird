...
fn main() -> Result<(), String> {
    ...
    if !(3..=4).contains(&args.len()) {
        return Err("usage: print_preview <photograph> <output-directory> [before-directory]".to_owned());
    }
    ...
    for (name, paper, roughness, white, black, surface_texture) in [
        ...
        for (view, scene) in [("default", scene), ("glare", Scene {
            ...
            let snapshot = Snapshot::pq(&samples, Size::<rawshim::px::Canvas>::measured(1280, 960));
