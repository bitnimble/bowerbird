...
fn main() {
    ...
    let mut noise: Option<(f32, f32)> = None;
    let mut args = std::env::args().skip(1);
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--noise" => {
                let spec = args.next().expect("alpha,sigma_sq");
                let (alpha, sigma_sq) = spec.split_once(',').expect("alpha,sigma_sq");
                noise = Some((
                    alpha.trim().parse().expect("a number"),
                    sigma_sq.trim().parse().expect("a number"),
                ));
            }
    ...
    if files.is_empty() {
        eprintln!("demosaic_psnr [--noise alpha,sigma_sq] <image>...");
        return;
    }
    ...
    for path in &files {
        ...
        let quad = [0u32, 1, 1, 2];
        let cfa = rawshim::cfa::Cfa::bayer(quad).expect("RGGB is a pattern");
        let colour_at = |r: usize, c: usize| usize::from(cfa.colour_at(r, c));

        // Poisson-Gaussian noise on the mosaic, at the shape `galosh::NoiseModel` describes: the
        // variance of a sample reading `s` is `alpha * s + sigma_sq`.
        //
        // **Off by default, because the question it answers is a different one.** A clean mosaic
        // measures the interpolation; a noisy one measures how the direction decision holds up when
        // the differences it steers by are partly grain, which is the only setting in which running
        // those decisions in a variance-stabilised domain can be worth anything. The level to ask
        // about is not a raw sensor's - RCD runs after the denoise here - but the residual that
        // survives it.
        // **Unit variance, which three uniforms are not.** Their sum has variance 3/12, so the
        // centred value carries a standard deviation of a half and the noise injected below would
        // be a quarter of the variance this flag names.
        const TRIPLE_SIGMA: f32 = 0.5;
        ...
        let mut gauss = || {
            ...
            for _ in 0..3 {
                ...
                seed ^= seed >> 7;
                seed ^= seed << 17;
                sum += (seed >> 40) as f32 / 16777216.0;
            }
            (sum - 1.5) / TRIPLE_SIGMA
        };
        ...
        for r in 0..h {
            for c in 0..w {
                ...
                if let Some((alpha, sigma_sq)) = noise {
                    let variance = alpha * mosaic[r * w + c] + sigma_sq;
                    mosaic[r * w + c] =
                        (mosaic[r * w + c] + gauss() * variance.max(0.0).sqrt()).clamp(0.0, 1.0);
                }
            }
        }
        ...
        let Some(out) = pollster::block_on(rawshim::demosaic::demosaic_plane(
            gpu,
            rcd,
            &uploaded,
            &cfa,
        ...
        // The reconstruction as a picture, where the question is what the demosaic did to the
        // *palette* rather than to the mean squared error. PSNR cannot see a colour set that has
        ...
        let flat = bilinear(&mosaic, w, h, quad);
