...
fn main() {
    for path in std::env::args().skip(1) {
        ...
        let Some(whole) = rawshim::decode_frame(&path, 0) else {
        ...
        let started = std::time::Instant::now();
        let bounded = rawshim::decode_frame(&path, FULL_RENDITION);
        let bounded_ms = started.elapsed().as_millis();
        let halved = bounded.as_ref().is_some_and(|f| f.reduced > 1);
