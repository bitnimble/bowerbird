...
fn main() {
    ...
    for path in &files {
        ...
        let checksum = match &frame.pixels {
            ...
            // `decode_frame` reads the frame back; only the pipeline's own spellings keep it.
            rawshim::frame::Pixels::Resident(_) => unreachable!("this decode returns host samples"),
        };
        drop(frame);
