...
- The open is `native/rawshim/src/edit.rs`. A server calls it natively; a browser
  calls it through `native/rawshim/src/wasm.rs`, the wasm32 build of the same
  crate, in a worker. So the open is Rust either way and the same Rust.
- The tick is too. `wasm.rs` holds the prepared frame on its own device, takes
...
- **wgpu in Rust (§6.2) was the answer on both hosts.** It shipped on the server
  first, with the page orchestrating the same WGSL through its own `GPUDevice`;
  that arrangement was two orchestrations of one picture and they drifted, which
  is what §6.3's parity pin existed to catch. wgpu 30 can open a surface on an
  `OffscreenCanvas`, so the page hands its canvas to the module and there is one
  orchestration. `gpu_fixture.rs` pins the graded frame; nothing on the page
  builds a uniform or records a pass.
