...
- [x] **The RAW is what crosses, and the prepared frame is gone** (`b8646f2`, `a549964`). 72MB
      ...
      `bb_prepare_edit_*` C ABI are deleted; `web/e2e/decode/local_decode.spec.ts` asserts the editor
...
- [x] **The client does the open** (`b8646f2`). `prepareRaw(bytes, request)` returns `edit::encode`'s
      ...
      **The decode itself is done and proven in a browser** (`3542685`): `web/e2e/decode/local_decode.spec.ts`
...
- [x] **The colour Detail slider is interactive** (`f453757`). Superseded with the denoiser it
      ...
      denoise has no such split - both amounts enter Phase 5 and the chroma pyramid - so a colour
      change re-prepares like a luminance one.
- [x] **A Detail slider draws its answer a band at a time.** Superseded, and worth reading as
      history: this was built for the editor's own denoise, which no longer exists. The banding was
      of `DenoiseChain.record`'s eight passes over the prepared frame, with `yuv_sigma_norm`'s
      in-place scaling deciding the halo's shape and a 112-row band chosen to land on both
      `pass12`'s tiles and the regression's workgroups.

      What replaced it bands the *whole chain below the mosaic* instead - denoise, demosaic, code,
      warp, sharpen - because that is where the seconds are once the editor denoises the mosaic
      like a rendition. `HeldRaw::band` cuts a strip through `tile::Source::Held`, which is the
      loupe's path with the read already done, and
      `a_band_of_a_held_mosaic_is_the_frame_it_was_cut_from` pins a band bit-for-bit against the
      whole frame.

      **The grid-anchoring defect outlived the rewrite.** `pass12` shrinks within a tile measured
      from its region's own origin, so a window starting off that lattice filters every pixel
      against a different neighbourhood. `denoise_in_tiles` had rounded its own tiles to it since
      the tiled decode; nothing else did, and the first banded prepare was not the frame until
      `galosh::SHRINK_LATTICE` was named and `Held::window` rounded to it. `galosh_band.rs` still
      pins the kernel-level claim on a synthetic frame, so a failure says which kernel.
...
- [x] **Delete what only exists to cross a wire**: `bb_prepare_edit_*`, `rawshim_edit.ts`, the
      ...
      **And then the framing itself**, once the tick moved into the module below. A frame no
      longer crosses *any* boundary - not a wire, not the worker's - so `edit::encode`, its
      padding, `PreparedHeader`'s `edits`, `detail`, `colour` and `samples_len`, and
      `prepare_bytes_async` are gone with it. What stays is `edit::prepare_bytes`, which reads the
      samples back for the fixture suite and the benchmarks, and a header that is what the panel
      shows.

- [x] **The tick moved into the module, and the page's own orchestration is gone.** This was the
      last thing written twice: `edit_pipeline.ts` recorded the same passes over the same WGSL that
      `gpu.rs` records, and the two drifted - which is what `gpu_parity.spec.ts` existed to catch,
      and why a frame had to be read back out of the worker at all.

      wgpu 30 can open a surface on an `OffscreenCanvas`, so the page hands its canvas to the
      worker (`transferControlToOffscreen`) and `wasm.rs` draws onto it from the device the frame
      is already on. A tick is now a region, an exposure, an `Adjust` and a `Geometry` crossing as
      JSON; nothing else does. The loupe's tile is built and kept on that device too, and a Detail
      band is copied buffer to buffer into the frame rather than shipped out and uploaded back.

      Deleted with it: `edit_pipeline.ts`, `shaders.ts`'s WGSL composition and uniform writer,
      `pass_timer.ts`, `gpu_parity.{html,spec.ts}` and the four TS pins that held the page's copy
