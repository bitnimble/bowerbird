...
**What each stage costs is measured here rather than quoted** (`render_benchmark.ts`). The panel opens on an estimate taken from `bench.budget.json`, which is one machine's answer about one camera; the Measure button renders one of the library's own photographs several times, once per configuration per round, and prices each stage as the difference the fastest round makes. A difference rather than a lap because several stages share a dispatch - the coding and the defringe are one pass, and so are the resize, the warp and the sharpen - so no lap exists that prices either of them alone. The fastest round rather than the mean for the reason `bench_stages` gives: contention only ever adds, and a single round on a busy machine priced two of the five at nothing.

Those numbers land in `render_timings`, a row per library and rendition. A table rather than a field on `libraries` or a blob in `settings`, and the reason is the run's own length: a `max` benchmark is minutes, so a shared value read at the start and written back at the end is a value the next benchmark to finish silently overwrites. A row each cannot lose one. They are not replicated, a measurement describing this machine's hardware rather than the library, and they cascade away with it.
