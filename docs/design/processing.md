...
**A library may also leave stages out of those two renders**, per rendition, through `render_skip_full` and `render_skip_max` (`schemas/render_stages.ts`): the dust removal, the denoise, the camera match, the defringe and the sharpen. The camera match is most of what a render costs - measured at 394ms of an 818ms `full` on a 24MP frame - so a catalogue of proof sheets can have its imports back in a third of the time, and an archive can leave every stage on. Not retroactive, like the two settings above it.

Five and no more, because these are the five the renderer is already gated on: the denoise at 0 stops at the noise fit (`galosh::wanted`), the sharpen and the defringe at 0 are refused by `image::Strengths`, the dust switch is `dust::Settings::wanted`, and the camera match is `match_embedded_jpeg`. So the setting turns a job field down rather than opening a second path through the pipeline, and a stage nobody has gated is a stage this cannot offer.
...
**What each stage costs is measured here rather than quoted** (`render_benchmark.ts`). The panel opens on an estimate taken from `bench.budget.json`, which is one machine's answer about one camera; the Measure button renders a photograph several times, once per configuration per round, and prices each stage as the difference the fastest round makes. A difference rather than a lap because several stages share a dispatch - the coding and the defringe are one pass, and so are the resize, the warp and the sharpen - so no lap exists that prices either of them alone. The fastest round rather than the mean for the reason `bench_stages` gives: contention only ever adds, and a single round on a busy machine priced two of the five at nothing.

**The figures belong to the machine, not to a catalogue, so they are scaled to one sensor before they are kept**: 6000x4000, a common full-frame mirrorless, which is also the frame the shipped estimates were taken on (`scaledToReference`). A stage is a pass over the frame, so its cost goes with the frame's area; the roughest fit is the camera match, whose fit is over a fixed number of pairs however large the sensor. Without the scaling, what a stage costs would depend on which body the photograph the benchmark found came from, and the same machine would quote a different answer per library.
