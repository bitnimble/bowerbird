...
function serve(dbPath: string, port: number): Subprocess {
  return spawn(['bun', 'run', 'src/index.ts'], {
    cwd: ROOT,
    // `BOWERBIRD_UPDATE_REPO` empty turns update checking off (§23.5): left on, the shell's
    // sidebar grows a row the moment a release exists that is newer than this checkout, and
    // the suite starts depending on what is published.
    env: {
      ...process.env,
      DB_PATH: dbPath,
      PORT: String(port),
