...
//
// The scan and the watcher both need it, and answering it separately means
...
export interface LibraryScope {
  ...
  /** Whether JPEG, PNG, HEIC and AVIF files are photographs here (`importsFormat`). */
...
export function libraryScope(
  library: Pick<Library, 'root_path' | 'include_subfolders' | 'include_non_raw' | 'bin_name'>,
  excluded: ReadonlySet<string>,
): LibraryScope {
  return {
...
/** Location only: whether the *format* is one this library takes is `importsFormat` (§7). */
