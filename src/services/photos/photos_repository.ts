...
export class PhotosRepository {
  ...
  // still only cleared for rows proven present, which a binned row is not.
    ...
    this.db
      .query(
        `UPDATE photos SET file_path = ? || substr(file_path, ?)
           WHERE library_id = ? AND is_deleted = 1 AND deleted_from_path = file_path
  ...
  rewriteBinnedPathPrefix(libraryId: string, oldPrefix: string, newPrefix: string): void {
    ...
    this.db
      ...
      .run(newPrefix, oldPrefix.length + 1, libraryId, lo, hi);
  }
  ...
  // `needs_tile` and `needs_renditions` go back to 1 because `markDeleted` zeroed
  // them: without this a photograph that comes back out of the Bin having never
  // had its renditions built would sit unbuilt for ever, since nothing revisits a
  // row that is not flagged.
  markRestored(id: string, filePath: string): void {
    this.db.transaction(() => {
      this.db
        .query(
          `UPDATE photos SET is_deleted = 0, file_path = ?, deleted_from_path = NULL, is_missing = 0,
             needs_tile = 1, needs_renditions = 1 WHERE id = ?`,
        )
        .run(filePath, id);
      this.refreshStackOf(id);
    })();
  ...
  listBinnedForSync(libraryId: string): SyncDbPhoto[] {
    return this.mapSyncRows(
      this.db.query(`SELECT ${SYNC_COLUMNS} FROM photos WHERE library_id = ? AND is_deleted = 1`).all(libraryId) as SyncRow[],
    );
  }
  ...
  //
  // Photos whose develop settings are newer than the files built from them, narrowed
  // to `photoIds` where the caller has some in mind.
  //
  // **The predicate is the mechanism, not a filter on one.** Asking "are the edits
  // newer than the render" is true however the photo got that way - a save, an undo,
  // a tab closed mid-edit, a process killed between the two - so the same query
  ...
  // requests for it to be correct.
  //
  // Self-clearing: a rebuild stamps `renditions_built_at` later than the
  // `updated_at` that queued it, so a photo stops matching as soon as it is done.
  queueEditedSince(photoIds?: readonly string[]): number {
    const narrowed = photoIds == null ? '' : `AND p.id IN (${photoIds.map(() => '?').join(', ')})`;
    if (photoIds != null && photoIds.length === 0) return 0;
    return this.db
      .query(
        `UPDATE photos SET needs_tile = 1, needs_renditions = 1, processing_error = NULL
          WHERE id IN (
            SELECT p.id FROM photos p JOIN photo_edits e ON e.photo_id = p.id
             WHERE p.is_missing = 0 AND p.is_deleted = 0 ${narrowed}
               AND (p.renditions_built_at IS NULL OR e.updated_at > p.renditions_built_at)
          )`,
      )
      .run(...(photoIds ?? [])).changes;
  }
