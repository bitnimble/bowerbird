...
const log = new Logger('daily-scan');
...
// Runs a full scanAll once a day at a configured local time: the backstop for
...
export class DailyScan {
  ...
  private async fire(): Promise<void> {
    this.schedule(); // book tomorrow first, so a long scan can't skip a day
    if (this.running) {
