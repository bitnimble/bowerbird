...
beforeEach(() => {
  ...
  // As `db/connection.ts` opens it. Enforcement is off by default and is per connection, so
  // without this the cascade these tables rely on never fires and a test asserting it would pass
  // against nothing.
...
describe('PhotoEditsRepository replication', () => {
  ...
  it('moves the stamp on every write, and not on a save that changes nothing', () => {
    save({ exposure: 1.5 });
    const first = stored();
    expect(first).not.toBeNull();

    repo.save(PHOTO, edited({ exposure: 1.5 }), 1);
    expect(stored()).toBe(first);

    repo.undo(PHOTO, 1);
    const undone = stored();
    expect(undone! > first!).toBe(true);
