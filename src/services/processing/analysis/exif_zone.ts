...
// A JPEG's EXIF is an APP1 segment carrying "Exif\0\0" and then a whole little TIFF, so once it
// is found the same walk as everything else reads it.
function findJpegExif(bytes: Uint8Array, view: DataView): Uint8Array | null {
  if (view.getUint16(0, false) !== 0xffd8) return null;
  let at = 2;
  while (at + 4 <= bytes.byteLength) {
    if (view.getUint8(at) !== 0xff) return null;
    const marker = view.getUint8(at + 1);
    // Start of scan: the entropy-coded data begins, and no header segment follows it.
    if (marker === 0xda) return null;
    const length = view.getUint16(at + 2, false);
    const body = at + 4;
    if (marker === 0xe1 && ascii(view, body, 4) === 'Exif') {
      // "Exif" then two NULs, then the TIFF block.
      return bytes.subarray(body + 6, Math.min(at + 2 + length, bytes.byteLength));
    }
    at = at + 2 + length;
  }
  return null;
...
// A PNG's is an `eXIf` chunk, which is the TIFF block on its own.
function findPngExif(bytes: Uint8Array, view: DataView): Uint8Array | null {
  if (bytes.byteLength < 16 || view.getUint32(0, false) !== 0x89504e47) return null;
  let at = 8;
  while (at + 8 <= bytes.byteLength) {
    const length = view.getUint32(at, false);
    const type = ascii(view, at + 4, 4);
    if (type === 'eXIf') return bytes.subarray(at + 8, Math.min(at + 8 + length, bytes.byteLength));
    // Nothing after the pixels is worth walking for, and IEND is the end.
...
export function parseCaptureOffset(bytes: Uint8Array): string | null {
  ...
  return (
    readTiff(bytes) ??
    readTiff(findJpegExif(bytes, view)) ??
    readTiff(findPngExif(bytes, view)) ??
    readTiff(findExifBlock(bytes, view, 0, bytes.byteLength, 0))
  );
}
