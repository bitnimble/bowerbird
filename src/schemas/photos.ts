...
export const PhotoSummarySchema = z.object({
  ...
  //
  // The recipe's single input, and so null for a row composed from several - which has a name
  // rather than a path, and no one file a reader could be shown.
  ...
  // rather than the client joining the shoots list to find out - which is what the badge on its tile
  // reads, a grid being able to hold the hidden beside the live.
  is_hidden: z.boolean(),
  ...
  // What this row is composed as, where it is composed out of other photographs rather than
  // imported from a file (§19.4). Everything else about it is an ordinary photograph's: it draws
  // from its own renditions, versioned by its own `tile_built_at`, and lays out at its own
  // `width`/`height`, which for a composite is the canvas already framed to what its frames cover.
  // What this adds is the badge, and the band of frames the badge opens.
  ...
  // "edited" is not otherwise visible from a picture that has been edited well.
  is_edited: z.boolean(),
  // Whether any row this one is composed *from* has develop settings stored, which is a
  // different question and the one `shown_rendition` asks of a panorama: a canvas carries its
  // own document whichever picture it is composited from - the framing the merge wrote is a
  // crop, and a crop applies to the cameras' pictures as readily as to a render - where a
  // frame's document is an edit no JPEG of that frame has in it. False for a photograph, which
  // composes nothing.
  frames_edited: z.boolean(),
  // Which rendition to draw this photo from right now, decided from the viewer's setting
  // against what is actually on disk - so a client asks nothing before it can show a
  // photo, whether it is the one just opened or a neighbour the viewer is holding ready.
  // Resolved by the service, which holds the library and the settings; the repository
  // knows neither (§18.5).
  //
  // A listing draws on no stat at all: `best_available` is the one mode that may, and only
  // the detail has statted every file to answer `renditions` below, so a row answers it from
  // the promise instead. Every input - the **library's** rendition source, the setting, this
  // photo's remembered choice and whether it is edited - is the same on both, because a row
  // that answered differently from the detail of the same photograph would open it one way
  // from the grid and another from a deep link. Both come out of the one function in
  // `photo_read_service.ts`.
  //
  // The library's source and not the photo's own, which is a distinction worth keeping: a
  // photograph is promised what its library serves, and reading its own instead makes the
  // answer a fact about a file rather than a promise - which is what the 404 that heals a
  // missing rendition depends on (§18.5).
  shown_rendition: ViewerRenditionSchema,
  // Whether this photograph has a camera JPEG behind it at all (§7): a RAW's is lifted out of
  // it, a JPEG *is* one, and a PNG, HEIC or AVIF has none. On the summary rather than derived by
  // the client from `file_path`, because that would be the extension set spelled a third time
  // and in a second language - and a client that guesses wrong offers a file that cannot exist.
  has_embedded: z.boolean().default(true),
});
export type PhotoSummary = z.infer<typeof PhotoSummarySchema>;

export const PhotoSummaryListSchema = z.array(PhotoSummarySchema);

export const PhotoDetailSchema = PhotoSummarySchema.extend({
  // How this photograph's pixels are arrived at, and what from: an ordinary photograph is a
  // `file` naming its own path, and a synthesised one composes other rows.
  ...
  // Whether the RAW itself is on this device (docs/replication.md §7.5). False on a
  // replica that holds the catalogue and not the bytes, where the pictures come from
  // a peer's renditions and only the editor actually needs the file.
  has_original: z.boolean(),
  // The rendition `shown_rendition` would rather show than what it settled for, because
  // this photo does not have it yet - `max` is built on request, and a render in a
  // library that serves the camera's JPEG is too. Null once shown and wanted agree,
  // which is the ordinary case: the viewer builds this before it shows the photo, then
  // never asks again. Detail-only: a build is worth starting for the photo being read,
  // never for a neighbour nobody has stepped to.
  rendition_to_build: ViewerRenditionSchema.nullable(),
  // Whether the rendition being shown was built from develop settings this photograph
  // has moved past - an editor closed without the client getting to say so, or a save
  // that landed after the last render. About `shown_rendition` alone: the reader is
  // being told what is on their screen is behind, and the others are rebuilt or swept
  // when they are next asked for. False for the camera's JPEG, which no edit changes.
  rendition_stale: z.boolean(),
  ...
  renditions: z
    .record(
      ...
      z.object({
        // Null for the camera's JPEG of a row that has no file of its own to lift one out of.
...
export const PhotoListResponseSchema = z.object({
  ...
  // Photographs, a stack counted as all of its members, where `total` counts it
  // once (§19.5.1). Answered and withheld with `total`, off the same scan.
  photo_total: z.number().int().optional(),
  offset: z.number().int(),
  limit: z.number().int(),
...
// The two fields a cull sets, over a whole selection (§18.3.1). Not the rest of
// an update: notes and the rendition a photo reopens at are each about one
// photograph, and setting either over a burst is nothing anyone means to ask for.
export const PhotoMarksSchema = UpdatePhotoRequestSchema.pick({ rating: true, triage: true });
export type PhotoMarks = z.infer<typeof PhotoMarksSchema>;
...
export const PhotoPositionsResponseSchema = z.record(z.string(), z.array(z.number().int().min(0)));
export type PhotoPositionsResponse = z.infer<typeof PhotoPositionsResponseSchema>;

// Which bodies and lenses a collection was actually shot with, so the filter menu
// offers those and not every model in the catalogue.
//
// The filters are the view's own - the Bin's soft-deleted rows, the missing view's
// gone ones - and never the chips the reader is ticking: a body the gallery cannot
// be narrowed to is a tick that empties the grid, while a list that shrank as it was
// ticked could not be un-ticked back. What the ticks do to each other is the pairing
// below, which does not move.
export const PhotoModelsRequestSchema = z.object({
  scope: PhotoSelectionFields.shape.scope,
  filters: PhotoFiltersSchema.default({}),
});
export type PhotoModelsRequest = z.infer<typeof PhotoModelsRequestSchema>;

// The pairings rather than two lists: a lens that was never on a ticked body can
...
// How many photographs a collection holds on each day it holds any, which is what the
// calendar draws its density under and what tells it which month to open at. The view's
// own filters and not the reader's chips, for the reason the models list states.
export const PhotoDaysRequestSchema = z.object({
  scope: PhotoSelectionFields.shape.scope,
  filters: PhotoFiltersSchema.default({}),
});
export type PhotoDaysRequest = z.infer<typeof PhotoDaysRequestSchema>;

// Only the days that hold something, ascending. A decade of daily shooting is a few
// thousand rows, where a row per day in the range would be most of it empty.
export const PhotoDaysResponseSchema = z.object({
  days: z.array(z.object({ day: z.string(), count: z.number() })),
});
export type PhotoDaysResponse = z.infer<typeof PhotoDaysResponseSchema>;
...
export type MarkPhotosRequest = z.infer<typeof MarkPhotosRequestSchema>;

// Putting a selection away, or bringing it back. Not one of the marks: hiding carries its own
// stamp so that a rating arriving from a peer cannot undo it, and a batch of both would have to
// move two.
export const HidePhotosRequestSchema = z.object({ target: PhotoTargetSchema, hidden: z.boolean() });
export type HidePhotosRequest = z.infer<typeof HidePhotosRequestSchema>;

// Unbounded, unlike a request's `PhotoIdListSchema`: this is a whole selection spelled out.
export const PhotoIdsResponseSchema = z.object({ photo_ids: z.array(IdSchema) });
export type PhotoIdsResponse = z.infer<typeof PhotoIdsResponseSchema>;

export const RenditionJobResponseSchema = z.union([
  z.object({ job: z.null() }),
  z.object({ job: JobSchema, builtFrom: z.string().nullable() }),
]);
export type RenditionJobResponse = z.infer<typeof RenditionJobResponseSchema>;
