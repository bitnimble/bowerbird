...
// Both widths in circulation, and no width between them: ids were eight
// characters until peers began minting them offline, and every one of those rows
// keeps the id it was born with (`newId`).
export const IdSchema = z.string().regex(/^(?:[0-9a-z]{8}|[0-9a-z]{16})$/);
...
export const RENDITIONS = ['grid', 'full', 'max', 'embedded'] as const;
export const RenditionSchema = z.enum(RENDITIONS);

// The two halves of an import, which land at different times and are worth
// telling apart everywhere: the grid tile the gallery shows (~18ms), then the
...
export const DeletedCountSchema = z.object({ deleted: z.number().int() });
export type DeletedCount = z.infer<typeof DeletedCountSchema>;

export const QueuedCountSchema = z.object({ queued: z.number().int() });
export type QueuedCount = z.infer<typeof QueuedCountSchema>;
