...
/**
 * Whether the file says a WAL-mode database, which is what a server having it open looks like.
 *
 * `createDatabase` puts every catalogue into WAL as it opens and `Database.close` takes it back out,
 * so rollback mode in the header means nothing is serving from this file - and, unlike asking the
 * database, it is an answer no lock can confuse.
 */
export function isWalMode(dbPath: string): boolean {
  return readVersionByte(dbPath) === WAL_FORMAT;
}
 ...
 * every snapshot this app takes says read-version 1, as does every catalogue closed
 * cleanly - `Database.close` leaves rollback mode behind it. Either way the `-wal`
 * goes with the mode, so a rollback-mode header beside a non-empty one means the two
 * came from different databases - which is exactly the shape of a hand-rolled restore
 * that copied the snapshot in and forgot the sidecars (§4.9). It cannot false-positive:
 * SQLite removes the `-wal` when a database leaves WAL mode, so the combination is
 * never legitimate.
 */
...
// relies on, and applies migrations. FK enforcement is off by default and must be
// set per connection (DESIGN §4). WAL + a busy timeout let concurrent per-library
// syncs read while writes serialize.
export function createDatabase(dbPath: string): Database {
  refuseToReplaceAMissingCatalogue(dbPath);
  if (dbPath !== ':memory:') refuseAMismatchedWal(dbPath);
  const db = new Database(dbPath);
  // First, because the line below needs it. `Database.close` leaves a catalogue in rollback mode,
  // so this is a conversion rather than the no-op it is on a database already in WAL - and entering
  // WAL mode needs the database to itself. With no timeout set yet, two processes opening at once
  // have one of them fail outright instead of waiting the moment out.
