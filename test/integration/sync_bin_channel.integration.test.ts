...
import { renditionVariant } from '../../src/services/processing/renditions/renditions';
...
import type { PhotoStateRepository } from '../../src/services/photos/mutations/photo_state_repository';
...
let scanRepository: PhotoScanRepository;
let state: PhotoStateRepository;
...
const rows = (): Row[] =>
  db
    .query(
      `SELECT id, json_extract(recipe, '$.path') AS file_path, deleted_from_path, is_deleted, is_missing FROM photos`,
    )
    .all() as Row[];
const only = (): Row => {
  const all = rows();
...
beforeEach(() => {
  ...
  paths = photoPaths(db);
  state = photoState(db);
  scanRepository = photoScan(db, photos);
  scan = new ScanService(
    scanRepository,
    paths,
    photoMetadata(db, photos),
    photos,
    libraries,
...
// photograph on every scan, for ever.
...
/**
 * Removing a folder from the library takes its binned rows and leaves their files
 * (§4.7), so the only thing keeping those files out of the catalogue afterwards is
 * the rule written beside them - and the bin is walked with no exclusions at all,
 * on purpose, since an excluded folder's binned frames are still binned.
 *
 * Without the restore path being tested, every one of them comes back on the next
 * scan as a *new* already-binned photograph, under a new id that the removal's
 * tombstones do not name - so it replicates to peers that have already applied the
 * removal, and it happens again on every scan.
 */
test('removing a folder does not re-import its binned frames as new photographs', async () => {
  makeLibrary();
  mkdirSync(abs('Trip'), { recursive: true });
  mkdirSync(abs('Bin/Trip'), { recursive: true });
  copyFileSync(FIXTURE, abs('Trip/live.arw'));
  copyFileSync(FIXTURE, abs('Bin/Trip/old.arw'));
  await scan.scanLibrary(LIB);
  expect(rows().filter((r) => r.is_deleted === 1)).toHaveLength(1);
  const shootId = shootOf(rows().find((r) => r.is_deleted === 0)!.id)!;

  // The real path, so the rule this depends on is the rule that removal writes.
  const shoots = new ShootsRepository(db);
  const rules = new FolderRulesRepository(db);
  await new ShootsService(shoots, paths, state, libraries, rules).delete(shootId, 'remove');
  await scan.scanLibrary(LIB);

  expect(rows()).toEqual([]);
  expect(existsSync(abs('Bin/Trip/old.arw'))).toBe(true);
  expect(existsSync(abs('Trip/live.arw'))).toBe(true);
});
...
// detection runs on a scoped scan even though the bin's walk and diff do not -
// without it the bin's files are unclaimed live additions and every binned RAW
// gets a second, live row.
test('a bin rename the watcher reports is followed on a scoped scan', async () => {
  makeLibrary();
  copyFileSync(FIXTURE, abs('a.arw'));
  await scan.scanLibrary(LIB);
  await service.delete([only().id]);
  ...
  renameSync(abs('Bin'), abs('Rubbish'));
  await scan.scanLibrary(LIB, { paths: ['Rubbish'] });
...
test('a scoped scan touches no binned row', async () => {
  makeLibrary();
  copyFileSync(FIXTURE, abs('a.arw'));
  copyFileSync(FIXTURE, abs('b.arw'));
  await scan.scanLibrary(LIB);
  ...
  await scan.scanLibrary(LIB, { paths: ['b.arw'] });
