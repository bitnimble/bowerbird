...
/** Rewrite when a backup's newest migration was applied, to stand in for another build's. */
function pretendMigratedAt(file: string, millis: number): void {
  const copy = new Database(file);
  try {
    copy.exec(`UPDATE __drizzle_migrations SET created_at = ${millis}`);
  } finally {
    copy.close();
  }
}
...
test('a backup from a newer Bowerbird is refused rather than restored', async () => {
  addLibrary(LIB, 'holiday');
  const { path: file } = await backups.backup(7);
  pretendMigratedAt(file, latestMigrationMillis() + 1);
  db.close();
...
test('a catalogue too corrupt to open can still be restored over', async () => {
  ...
  expect(version).toBe(latestMigrationMillis());
  db = createDatabase(dbPath);
...
/**
 * The file that lands is already the one the peers will take.
 *
 * Every fallible step - migrating the snapshot forward, lifting its rows above the
 * clock of the catalogue it replaces - happens to the staged copy, so by the time
 * anything is renamed there is no such thing as a catalogue that is restored and
 * not yet re-stamped. That state is what the marker file, the deferral, and the
 * sessions refused while one was pending all existed to survive.
 */
test('lands a catalogue already lifted above the clock it replaced', async () => {
  // The live catalogue, a week further on: stamps the peers already hold. Written
  // into the log directly, which is what the triggers would have produced.
  addLibrary(LIB, 'a week later');
  db.close();

  // The snapshot, as its own catalogue: what the person is rolling back *to*, with
  ...
  const { restamped } = await restoreBackup(dbPath, file);
  ...
  expect(stamps.stamp > before).toBe(true);
});
...
test('a WAL that cannot belong to the catalogue beside it is refused', async () => {
  ...
  // A restore done by hand: the snapshot copied in, the sidecars forgotten. SQLite binds a WAL to
  // nothing - its header names no database - so it is replayed over whatever it is found beside,
  // giving a catalogue that opens, passes `quick_check`, is the right size, and holds a mix of two.
  //
  // A snapshot rather than the catalogue itself, because `VACUUM INTO` always writes rollback mode,
  // where a catalogue's own mode rests on a close having won a switch it can lose (`driver.ts`) -
  // and the guard only fires on a rollback-mode header. The `-wal` does not have to be a real one:
  // the pairing is the whole check, and it reads a header byte and a file size.
  copyFileSync(file, dbPath);
  writeFileSync(`${dbPath}-wal`, 'not this catalogue’s');
  ...
  expect(refusal).toMatch(/cannot belong to/);
  // And it says what to do about it, which is the half that makes the refusal actionable.
