...
function walk(photos: PhotoNavigationRepository, ordering: Ordering, from: string): string[] {
  const seen = [from];
...
describe('stepping through a collection', () => {
  ...
  test('visits every member of a stack, where the listing shows one row for it', () => {
    const { db, photos, listing, stacks } = context;
    ...
    const listed = listing.listByLibrary(LIBRARY, 'taken_asc', 0, 100, NO_FILTERS);
    expect(listed.photos).toHaveLength(4);
  ...
  test('a photo the filter excludes still has neighbours, and they are adjacent to each other', () => {
    const { db, photos, listing } = context;
