// Scoped scan (the watcher's cheap path): reconciles the changed paths and
...
test('scoped add indexes the new file', async () => {
  await scan.scanLibrary(LIB, { paths: ['c.arw'] });
...
test('intra-dir rename, both halves in one window, is a move and not a re-add', async () => {
  ...
  await scan.scanLibrary(LIB, { paths: ['c.arw', 'd.arw'] }); // both halves, as the watcher names them
...
test('scoped delete flags missing but keeps the record', async () => {
  ...
  await scan.scanLibrary(LIB, { paths: ['d.arw'] });
...
test('cross-dir move reunites with the missing record via the move-source pool', async () => {
  ...
  await scan.scanLibrary(LIB, { paths: ['Trip/e.arw'] }); // the target event, in Trip's scope
...
test('a sibling the run was not told about is left alone', async () => {
  ...
  await scan.scanLibrary(LIB, { paths: ['Trip/e.arw'] });
  expect(row('Trip/unnamed.arw')).toBeNull();

  await scan.scanLibrary(LIB); // the backstop picks it up
