// Integration test for scan change-detection against a real Sony ARW fixture.
...
test('initial scan indexes the file with real LibRaw metadata', async () => {
...
test('a re-scan with nothing changed opens zero files (stat quick-check)', async () => {
  const before = opens;
...
test('a shoot folder renamed on disk relocates the shoot instead of orphaning its photos', async () => {
  ...
  // prefix rewrite can.
  db.query(
    `INSERT INTO photos (id, library_id, shoot_id, recipe, deleted_from_path, file_hash, width, height, orientation,
       date_added, file_size, is_deleted)
     VALUES (?, ?, ?, json_object('kind', 'file', 'path', ?), ?, 'h', 1, 1, 1, '2024-01-01', 1, 1)`,
...
test('sorting a shoot into a subfolder leaves the shoot where it is', async () => {
  ...
  expect(folderPath()).toBe('Renamed');
  // The photo moved rather than went missing; which shoot it now sits in is the
  // subfolder's own, and that is mirroring's business (sync_mirror).
  expect(row('Renamed/Selects/renamed.arw')?.is_missing).toBe(0);
});
