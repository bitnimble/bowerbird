...
import { photoPaths, photoScan } from './helpers/photo_repositories';
...
function makeLibrary(binName: string | null, readOnly = false): void {
  ...
  db.query(
    `INSERT INTO photos (id, library_id, recipe, deleted_from_path, width, height, date_added, is_deleted)
     VALUES (?, ?, json_object('kind', 'file', 'path', ?), 'Trip/a.arw', 100, 100, '2026-01-01T00:00:00.000Z', 1)`,
...
beforeEach(() => {
  ...
  service = new LibrariesService(libraries, photoScan(db), photoPaths(db));
});
...
test('renaming the bin leaves a formerly-read-only library\'s in-place rows alone', async () => {
  ...
  db.query(
    `INSERT INTO photos (id, library_id, recipe, deleted_from_path, width, height, date_added, is_deleted)
     VALUES (?, ?, '{"kind":"file","path":"Trip/b.arw"}', 'Trip/b.arw', 100, 100, '2026-01-01T00:00:00.000Z', 1)`,
  ).run(IN_PLACE, LIB);
