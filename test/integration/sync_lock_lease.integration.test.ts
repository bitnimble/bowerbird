// The scan lease's one unique contribution is cross-process exclusion (§9.7), and
...
// one database file, told to scan one library at once.
...
async function run(role: 'hold' | 'scan'): Promise<string[]> {
...
test('scanAll re-attempts a library it found locked', async () => {
  const db = createDatabase(dbPath);
  let refusals = 1;
...
test('a scan whose lease was taken over rolls its apply back', async () => {
  const db = createDatabase(dbPath);
  ...
  await expect(scan.scanLibrary(LIB)).rejects.toThrow(/lease/);
  const { count } = db.query('SELECT COUNT(*) AS count FROM photos').get() as { count: number };
  expect(count).toBe(0);
