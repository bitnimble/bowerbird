 ...
 *
 * Both halves are asked for rather than written down, and both fail the same silent way: a length
 * that is not the size this build writes is dropped by `descriptorsOf`, and a first byte that is
 * not the format tag makes the comparison score every pair as unalike. Either one leaves detection
 * finding nothing, with nothing thrown and nothing logged.
 */
function descriptor(): Buffer {
  const bytes = Buffer.alloc(descriptorSize());
  bytes[0] = descriptorFormat();
  return bytes;
}
...
function insertPhoto(
  ...
  db.query(
    `INSERT INTO photos (id, library_id, shoot_id, recipe, width, height, date_taken, date_added)
...
// The sort `setUp` gives this library, and so the one a band of it is listed in.
...
describe('stacks', () => {
  ...
  test('unstacking releases the photos and detection never reclaims them', () => {
    ...
    // Named by one of its photographs, which is all a selection ever resolves to.
    expect(stacks.unstackAllOf([ids[0]!])).toBe(1);
  ...
  // The bulk bar's Unstack, which is about a selection rather than about one
  // stack: it names photographs, and every stack any of them is in comes apart.
  test('unstacking a selection takes apart every stack in it and leaves loose photos alone', () => {
    const { db, stacks, repo, photos } = context;
    const first = [1, 2].map((n) => insertPhoto(db, n, { minute: n }));
    const second = [3, 4].map((n) => insertPhoto(db, n, { minute: n }));
    const loose = insertPhoto(db, 5, { minute: 5 });
    const stackA = stacks.create(first);
    const stackB = stacks.create(second);

    expect(stacks.unstackAllOf([first[0]!, second[1]!, loose])).toBe(2);

    expect(repo.get(stackA.id)).toBeNull();
    expect(repo.get(stackB.id)).toBeNull();
    expect(photos.listByLibrary(LIBRARY, 'taken_desc', 0, 100, NO_FILTERS).total).toBe(5);
  });

  test('unstacking a selection with no stack in it does nothing', () => {
    const { db, stacks } = context;
    const loose = insertPhoto(db, 1, { minute: 1 });
    expect(stacks.unstackAllOf([loose])).toBe(0);
  });
  ...
  test('rejecting the member a stack stands for moves the flag itself', () => {
    ...
    photoState(db).update(ids[2]!, { triage: 'untriaged' });
    expect(flagged()).toBe(ids[2]!);
  });
  ...
  test('a stack whose members are all rejected still has exactly one flagged member', () => {
    ...
    for (const id of ids) photoState(db).update(id, { triage: 'rejected' });
  ...
  test('binning the member a stack stands for moves the flag too', () => {
    ...
    photoPaths(db).markRestored(ids[2]!, 'IMG_3.ARW');
    expect(flagged()).toBe(ids[2]!);
  });
  ...
  test('a stack that survives losing a member still has one', () => {
    ...
    expect(repo.memberIds(source.id, ORDERING).sort()).toEqual([ids[0]!, ids[1]!, ids[2]!].sort());
  ...
  test('stacking photos already in a stack moves them and cleans up behind them', () => {
    ...
    expect(repo.memberIds(merged.id, ORDERING).sort()).toEqual([first[0]!, ...second].sort());
  });
  ...
  test('removing a photo that belongs to another stack leaves it where it is', () => {
    ...
    expect(repo.memberIds(other.id, ORDERING).sort()).toEqual([...theirs].sort());
  });
  ...
  test('removing nothing leaves an automatic stack in detection hands', () => {
    ...
    db.query('UPDATE photos SET descriptor = ? WHERE library_id = ?').run(DESCRIPTOR, LIBRARY);
    stacks.detect(LIBRARY);
    const auto = repo.autoStackIds(LIBRARY);
    expect(auto).toHaveLength(1);
    expect(repo.memberIds(auto[0]!, ORDERING).sort()).toEqual([...mine].sort());
  ...
  test('a band reads in the order the viewer steps through it', () => {
    ...
    // Both directions of the collection's sort, because a band with an order of
    // its own agrees with one of them by accident and neither of them on purpose.
    for (const ordering of ['taken_asc', 'taken_desc'] as const) {
      const band = stacks.photosOf(stack.id, { ordering }).map((photo) => photo.id);
      const walked = photoNavigation(db)
        .neighboursInLibrary(LIBRARY, ordering, ids[0]!, 50, NO_FILTERS)
        .filter((photo) => photo.stack_id === stack.id)
        .map((photo) => photo.id);
      expect(band).toEqual(walked);
    }
  });
  ...
  test('a manual stack survives a detection pass', () => {
    ...
    expect(repo.memberIds(stack.id, ORDERING).sort()).toEqual([...ids].sort());
  });
  ...
  test('detection groups identical frames, and only within the window', () => {
    ...
    expect(repo.memberIds([...stackIds][0]!, ORDERING)).toHaveLength(2);
