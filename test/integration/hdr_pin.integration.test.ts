...
import { hdrArgv, type HdrOptions } from '../../src/services/processing/rawshim_ops';
import { gradedSummary, type GradeSpec } from '../../src/services/processing/rawshim_debug';
...
test(
  ...
  () => {
    ...
    for (const [label, withMatch, maxEdge, peakNits] of [
      ...
      // Everything recorded below is computed where the samples are. The digest is a
      // SHA-256 for that reason and no other: these rows were generated by the
      // TypeScript that used to do this, so keeping the function keeps them valid
      // unregenerated - the pin still says "unchanged since before the port".
      //
      // `withMatch` fits the camera's colour the way a job with an SDR rendition
      // does: the SDR fit supplies the geometry, and the colour is refitted in the
      // grade's own domain.
      const graded = gradedSummary(FIXTURE, grade, { withMatch });

      rows.push(`${label}\tsize\t${graded.width}x${graded.height}`);
      rows.push(`${label}\tsha256\t${graded.sha256}`);
      // Per channel, because a shift in one is what a wrong matrix row looks like and
      // a whole-frame mean would hide it.
      const stats = graded.channels.map((c) => `${c.min}/${c.max}/${c.mean.toFixed(2)}`);
      rows.push(`${label}\tstats\t${stats.join(' ')}`);
      // A prime stride, so it walks all three channels and cannot land on a repeating
      // pattern.
      rows.push(`${label}\tsamples\t${graded.samples.join(',')}`);
    }
