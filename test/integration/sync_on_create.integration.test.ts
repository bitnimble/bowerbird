...
// LibrariesService starts a full scan through the lifecycle listener, without the
// create request waiting for the scan. Needs a real catalogue:
...
function buildServices(): { scan: ScanService; service: LibrariesService } {
  const photoProcessingRepo = photoProcessing(db);
  const photoScanRepo = photoScan(db, photoProcessingRepo);
  const photoPathsRepo = photoPaths(db);
  ...
  const service = new LibrariesService(new LibrariesRepository(db), photoScanRepo, photoPathsRepo);
  service.addLifecycleListener(scan);
  return { scan, service };
}
...
test('creating a library imports its photographs without a second request', async () => {
  const { scan, service } = buildServices();
  writeFileSync(path.join(root, 'photo.arw'), 'raw');

  const settled = new Promise<void>((resolve) => scan.onSettled(() => resolve()));
  ...
  const photo = db
    .query(`SELECT json_extract(recipe, '$.path') AS file_path FROM photos WHERE library_id = ?`)
