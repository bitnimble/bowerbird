...
describe('the tiles held', () => {
  const decoded = (): TileKeep => ({ left: 0, top: 0, width: 600, height: 600 });

  /** A decode that settles only when a test hands it an answer. */
  function pending(): {
    tiles: LoupeTiles;
    asked: string[];
    arrive: ((keep: TileKeep) => void)[];
  } {
    const asked: string[] = [];
    const arrive: ((keep: TileKeep) => void)[] = [];
    const tiles = new LoupeTiles(
      async (rect) => {
        asked.push(`${rect.left},${rect.top}`);
        return new Promise<TileKeep>((resolve) => arrive.push(resolve));
      },
      () => {},
    );
  ...
  /**
   * Only ever one tile is held, because only one is on the device.
   *
   * **The failure this exists for is silent and looks like a working magnifier.** The module keeps
   * exactly one tile (`HeldRaw::holdTile` replaces whatever was there), so a second entry here
   * would be a window `covering` answers with and the module cannot draw: the glass would report
   * itself sharp, and the region handed to the tick would be offset by *this* tile's origin
   * while the module drew through the other one - the reader magnifying a confidently wrong part
   * of the photograph.
   */
  test('holds only the tile the module is holding, so returning re-asks', async () => {
    const { tiles, asked, arrive } = pending();
    const sky = { x: 1000, y: 600 };
    const face = { x: 4000, y: 2600 };

    tiles.want(tileFor(sky, 400, FRAME));
    arrive[0]!(decoded());
    await Bun.sleep(0);
    expect(tiles.covering(sky, 400, FRAME)).not.toBeNull();

    tiles.want(tileFor(face, 400, FRAME));
    arrive[1]!(decoded());
    await Bun.sleep(0);

    // The sky's tile is not claimed to be here any more, because it is not on the device.
    expect(tiles.covering(face, 400, FRAME)).not.toBeNull();
    expect(tiles.covering(sky, 400, FRAME)).toBeNull();

    // So going back asks again rather than serving a window nothing can draw.
    tiles.want(tileFor(sky, 400, FRAME));
