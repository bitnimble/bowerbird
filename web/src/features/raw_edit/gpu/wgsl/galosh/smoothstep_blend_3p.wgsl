...
//
// `o32_smoothstep_blend_3p.comp`. The slider runs 0..3 across noisy, the LOESS baseline at
// half res, the quarter-res level upsampled, and the eighth - so its unit is *how far the
// colour is allowed to be smoothed*, in scales, rather than an amount of anything. The
// smoothstep is what keeps the crossings from showing as the slider moves.
...
@group(0) @binding(9) var<storage, read> c1_e_up: array<f32>;
@group(0) @binding(10) var<storage, read> c2_e_up: array<f32>;
@group(0) @binding(11) var<storage, read> c3_e_up: array<f32>;
...
fn smoothstep_blend_3p(@builtin(global_invocation_id) id: vec3u) {
  ...
  let d = vec3f(c1_e_up[p], c2_e_up[p], c3_e_up[p]);

  var blended: vec3f;
  if (pc.slider <= 0.0) {
    blended = a;
  } else if (pc.slider >= 3.0) {
    blended = d;
  } else {
    var lo: vec3f;
    var hi: vec3f;
