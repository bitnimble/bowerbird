...
describe('masonryBlockEnd', () => {
  // The same shapes, which at 1200 wide pack as [0, 1, 2] and [3, 4, 5].
  const RATIOS = [1.5, 1.5, 2 / 3, 1.5, 1.5, 1.5];
  const end = (minEnd: number, count = RATIOS.length): number =>
    masonryBlockEnd((i) => RATIOS[i]!, count, 1200, 240, minEnd);

  test('a block runs on to the end of the line its boundary falls in', () => {
    expect(end(1)).toBe(3);
    expect(end(2)).toBe(3);
  });

  test('a boundary already on a break is the break', () => {
    expect(end(3)).toBe(3);
  });

  test('a block always takes a line, however far its start has drifted', () => {
    expect(end(0)).toBe(3);
  });

  test('the collection running out ends the block mid-line, there being no line after it', () => {
    expect(end(4)).toBe(6);
