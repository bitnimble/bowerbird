...
// Two of these overlap routinely: the sync poll ticks once a second while a
// verdict is being set. Each used to snapshot positions the other had already
// moved and apply the same shift again, walking the selection off its photos.
test('two overlapping re-reads move the selection once, not twice', async () => {
  const { store, presenter } = await openAt(300);
  store.selection = SelectionRanges.of(20, 69);
  ...
  expect(store.selection.ranges).toEqual([{ start: 25, end: 74 }]);
  expect(store.rows.get(store.selection.ranges[0]!.start)?.id).toBe('p20');
});
...
test('a block whose re-read failed is not treated as confirmation', async () => {
  ...
  // on a stale row.
  for (const range of store.selection.ranges) {
    for (let index = range.start; index <= range.end; index++) {
      const held = store.rows.get(index);
      if (held != null) expect(held.id).toBe(`p${index - 5}`);
    }
  }
  expect(store.selection.size).toBeLessThanOrEqual(131);
});
