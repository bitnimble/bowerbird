 ...
 * held rows for *and* re-read. Inside it every photograph that is still there has
 * a sample, so the answer is exact and is read off them one position at a time:
 * an arrival **splits** a selected run, since it has no sample of its own and the
 * photographs either side of it keep theirs (three selected and one inserted
 * after the first leaves `{0} ∪ {2,3}`, not a run of four), and a photograph that
 * has gone has no sample either, so it simply leaves the selection.
 *
...
export function rebase(
  ...
  if (samples.every((sample) => sample.to === sample.from)) return selection;
  // Inside the domain the samples *are* the map: a selected position with a
  // sample goes where that sample went, and one without has been removed. Read
  // off them one at a time rather than walked as runs of a common shift - a shift
