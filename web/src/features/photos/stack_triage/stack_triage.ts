...
/**
 * The key that picks each half, in drawn order.
 *
 * Both pairs are bound at once, so a key never changes meaning when the
 * arrangement flips under a resize: `→` and `↓` both mean the second photograph
 * whatever is on screen. This is only which of the two to *show*, on the halves
 * and on the verdict bar, so it reads as the axis the pair is actually laid out
 * on.
 */
export function sideKeys(direction: 'row' | 'column'): [string, string] {
...
/**
 * The largest box of aspect `a` inside `w` by `h`: where flip's one photo goes,
 * as `arrangement` places split's two.
 *
 * The box the whole space when the aspect is not yet knowable, rather than
 * nothing: a stage of no extent never paints, so it never decodes, so the aspect
 * it was waiting for never arrives.
 */
export function fitted(a: number, w: number, h: number): Shape {
  if (!Number.isFinite(a) || a <= 0) return { width: w, height: h };
  const width = Math.min(w, h * a);
