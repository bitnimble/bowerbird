...
describe('arrangement', () => {
  ...
  test('the keys shown for the two halves point along the axis they are laid out on', () => {
    // Which the arrangement above decides, and a resize can flip. Only the hint
    // moves with it: both arrows of both axes are bound, so `↓` is the second
    // photograph whether or not the pair is stacked.
    expect(sideKeys(arrangement(1.5, 1.5, 1200, 400).direction)).toEqual(['←', '→']);
    expect(sideKeys(arrangement(1.5, 1.5, 400, 1200).direction)).toEqual(['↑', '↓']);
  });
});

describe('fitted', () => {
  test('keeps the aspect and touches whichever edge binds', () => {
    const wide = fitted(1.5, 1200, 400);
    expect(wide).toEqual({ width: 600, height: 400 });
    const tall = fitted(1.5, 300, 400);
    expect(tall).toEqual({ width: 300, height: 200 });
  });

  test('an aspect that is not knowable yet takes the whole box', () => {
    // A stage of no extent never paints, so it never decodes, so the aspect it
    // was waiting for never arrives.
    expect(fitted(Number.NaN, 800, 600)).toEqual({ width: 800, height: 600 });
    expect(fitted(0, 800, 600)).toEqual({ width: 800, height: 600 });
  });
});
