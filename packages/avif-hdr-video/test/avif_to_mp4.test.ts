...
import { avifToMp4, orientationOfAvif } from '../src/index';
...
const rotated = new Uint8Array(readFileSync(new URL('./rotated.avif', import.meta.url)));
...
test('irot carries into the video track display matrix', () => {
  const expected = [
    { angle: 1, clockwise: 270, matrix: [0, -1, 0, 1, 0, 0, 0, 64, 1] },
    { angle: 2, clockwise: 180, matrix: [-1, 0, 0, 0, -1, 0, 64, 64, 1] },
    { angle: 3, clockwise: 90, matrix: [0, 1, 0, -1, 0, 0, 64, 0, 1] },
  ] as const;
  for (const { angle, clockwise, matrix } of expected) {
    const avif = rotated.slice();
    find(avif, ['meta', 'iprp', 'ipco', 'irot'])[0] = angle;
